...
namespace com.rfilkov.components
{
    ...
    public class JointOverlayer : MonoBehaviour
    {
        ...
        [Tooltip("Smooth factor used for joint motion.")]
        [Range(0f, 100f)]
        public float motionSmoothFactor = 0f;
            ...
            if (kinectManager && kinectManager.IsInitialized())
            {
                ...
                if (kinectManager.IsJointTracked(userId, iJointIndex))
                {
                    ...
                    if (posJoint != Vector3.zero && overlayObject)
                    {
                        ...
                        overlayObject.position = motionSmoothFactor > 0f ?
                            Vector3.Lerp(overlayObject.position, posJoint, motionSmoothFactor * Time.deltaTime) : posJoint;
