...
namespace com.rfilkov.kinect
{
    ...
    public class NetClientInterface : DepthSensorBase
    {
        ...
        [Tooltip("Whether to get object-data frames from the server, if required by KinectManager.")]
        public bool getObjectFrames = true;

        [Tooltip("Whether to get object-index frames from the server, if required by KinectManager.")]
        public bool getObjectIndexFrames = true;
        ...
        private TcpNetClient objectDataFrameClient = null;
        private TcpNetClient objectIndexFrameClient = null;
        ...
        private TcpNetClient color2objIndexFrameClient = null;
        ...
        private ulong lastObjectDataFrameTime = 0;
        ...
        private ulong lastColor2objIndexFrameTime = 0;
        ...
        private bool savedSyncDepthAndColor = false;
        private bool savedSyncBodyAndDepth = false;
        ...
        public override KinectInterop.SensorData OpenSensor(KinectManager kinectManager, KinectInterop.FrameSource dwFlags, bool bSyncDepthAndColor, bool bSyncBodyAndDepth)
        {
            ...
            {
                Debug.LogWarning("Playback selected, but this is not applicable to network sensors. Ignoring...");
            }
            ...
            savedSyncDepthAndColor = isSyncDepthAndColor;
            savedSyncBodyAndDepth = isSyncBodyAndDepth;

            isSyncDepthAndColor = savedSyncDepthAndColor && ((dwFlags & KinectInterop.FrameSource.TypeColor) != 0);
            isSyncBodyAndDepth = savedSyncBodyAndDepth && ((dwFlags & KinectInterop.FrameSource.TypeColor) != 0);
            isFrameSyncNeeded = isSyncDepthAndColor || isSyncBodyAndDepth;
        ...
        public override bool UpdateSensorData(KinectInterop.SensorData sensorData, KinectManager kinectManager, bool isPlayMode)
        {
            ...
            if (colorImageData != null && sensorData.lastColorFrameTime != lastColorImageTime && !isPlayMode)
            {
                if (sensorData.colorImageTexture == null || sensorData.colorImageTexture.width != sensorData.colorImageWidth || sensorData.colorImageTexture.height != sensorData.colorImageHeight)
                {
                    if (sensorData.colorImageTexture != null)
                        KinectInterop.Destroy(sensorData.colorImageTexture);
            ...
            if((sensorData.depthImageTexture == null && sensorData.depthImage != null && 
                ...
                (sensorData.objectImageTexture == null && sensorData.objectIndexImage != null &&
                    (/**kinectManager.getObjectFrames == KinectManager.ObjectTrackingType.SegmentationTexture ||*/ kinectManager.getObjectFrames == KinectManager.ObjectTrackingType.ObjectTexture)))
            {
            ...
            {
                if (infraredImageTex == null || infraredImageTex.width != infraredImageWidth || infraredImageTex.height != infraredImageHeight)
                {
                    if (infraredImageTex != null)
                        KinectInterop.Destroy(infraredImageTex);
        ...
        protected override bool CreatePointCloudColorShader(KinectInterop.SensorData sensorData)
        {
            if (pointCloudResolution == PointCloudResolution.DepthCameraResolution)
            {
                ...
                if (depth2colorFrameClient == null)
                {
                    ...
                    //isSyncDepthAndColor = savedSyncDepthAndColor;
                    //isSyncBodyAndDepth = savedSyncBodyAndDepth;
                    //isFrameSyncNeeded = isSyncDepthAndColor || isSyncBodyAndDepth;
                }
        ...
        protected override void DisposePointCloudColorShader(KinectInterop.SensorData sensorData)
        {
            if(depth2colorFrameClient != null)
            {
                ...
                //isSyncDepthAndColor = savedSyncDepthAndColor && ((frameSourceFlags & KinectInterop.FrameSource.TypeColor) != 0);
                //isSyncBodyAndDepth = savedSyncBodyAndDepth && ((frameSourceFlags & KinectInterop.FrameSource.TypeColor) != 0);
                //isFrameSyncNeeded = isSyncDepthAndColor || isSyncBodyAndDepth;
            }
            else
            {
        ...
        // creates the color-cam object-index shader and its respective buffers, as needed
        protected override bool CreateColorObjectIndexShader(KinectInterop.SensorData sensorData)
        {
            bool bSuccess = base.CreateColorObjectIndexShader(sensorData);

            if (color2objIndexFrameClient == null)
            {
                color2objIndexFrameDecompressor = LZ4DecompressorFactory.CreateNew();

                color2objIndexFrameClient = new TcpNetClient(sbConsole, color2objIndexFrameDecompressor);
                color2objIndexFrameClient.ConnectToServer(serverHost, serverBasePort + (int)NetMessageType.Color2ObjectIndex, "tobjindex", null);
                color2objIndexFrameClient.ReceivedMessage += new ReceivedMessageEventHandler(Color2ObjectIndexFrameReceived);
            }
        ...
        // disposes the color-cam object-index shader and its respective buffers
        protected override void DisposeColorObjectIndexShader(KinectInterop.SensorData sensorData)
        {
            ...
            if (color2objIndexFrameClient != null)
            {
                color2objIndexFrameClient.ReceivedMessage -= Color2ObjectIndexFrameReceived;
                color2objIndexFrameClient.Close();
                color2objIndexFrameClient = null;
            }
        }
        ...
        private void InitNetClients(KinectInterop.FrameSource dwFlags)
        {
            try
            {
                ...
                if ((dwFlags & KinectInterop.FrameSource.TypeObject) != 0 && getObjectFrames)
                {
                    objectDataFrameClient = new TcpNetClient(sbConsole, null);
                    objectDataFrameClient.ConnectToServer(serverHost, serverBasePort + (int)NetMessageType.ObjectData, "object-data", null);
                    objectDataFrameClient.ReceivedMessage += new ReceivedMessageEventHandler(ObjectDataFrameReceived);
                }

                if ((dwFlags & KinectInterop.FrameSource.TypeObjectIndex) != 0 && getObjectIndexFrames)
                {
                    objectIndexFrameDecompressor = LZ4DecompressorFactory.CreateNew();

                    objectIndexFrameClient = new TcpNetClient(sbConsole, objectIndexFrameDecompressor);
                    objectIndexFrameClient.ConnectToServer(serverHost, serverBasePort + (int)NetMessageType.ObjectIndex, "object-index", null);
                    objectIndexFrameClient.ReceivedMessage += new ReceivedMessageEventHandler(ObjectIndexFrameReceived);
                }
        ...
        private void CloseNetClients()
        {
            try
            {
                ...
                if (objectDataFrameClient != null)
                {
                    objectDataFrameClient.ReceivedMessage -= ObjectDataFrameReceived;
                    objectDataFrameClient.Close();
                    objectDataFrameClient = null;
                }

                if (objectIndexFrameClient != null)
                {
                    objectIndexFrameClient.ReceivedMessage -= ObjectIndexFrameReceived;
                    objectIndexFrameClient.Close();
                    objectIndexFrameClient = null;
                    objectIndexFrameClient = null;
                }
        ...
        private void ColorFrameReceived(object state, ReceivedMessageEventArgs args)
        {
            ...
            try
            {
                if (sensorData.colorImageWidth != args.message.frameWidth || sensorData.colorImageHeight != args.message.frameHeight)
                {
        ...
        private void DepthFrameReceived(object state, ReceivedMessageEventArgs args)
        {
            ...
            try
            {
                if (sensorData.depthImage == null || sensorData.depthImage.Length != (args.message.frameWidth * args.message.frameHeight))
                {
        ...
        private void InfraredFrameReceived(object state, ReceivedMessageEventArgs args)
        {
            ...
            try
            {
                if (infraredImageWidth != args.message.frameWidth || infraredImageHeight != args.message.frameHeight)
                {
        ...
        private void BodyIndexFrameReceived(object state, ReceivedMessageEventArgs args)
        {
            ...
            try
            {
                if (sensorData.bodyIndexImage == null || sensorData.bodyIndexImage.Length != (args.message.frameWidth * args.message.frameHeight))
                {
        ...
        private void ObjectDataFrameReceived(object state, ReceivedMessageEventArgs args)
        {
            if (sensorData == null)
                return;
            ...
            try
            {
                ...
                {
                    sensorData.alTrackedObjects = new KinectInterop.ObjectData[0];
                ...
                lock (objectTrackerLock)
                {
                    ...
                    string sObjectFrameData = System.Text.Encoding.UTF8.GetString(args.message.frameData);
                    sensorData.trackedObjectsCount = KinectInterop.SetObjectFrameFromCsv(sObjectFrameData, "\t", sensorData, ref sensorData.alTrackedObjects,
                        ref sensorToWorld, bIgnoreZcoords, out rawBodyTimestamp);
                    sensorData.lastObjectFrameTime = currentObjectTimestamp = rawObjectTimestamp = args.message.timestamp;
                    //Debug.Log("ReceivedObjectTimestamp: " + rawObjectTimestamp);

                    lastObjectDataFrameTime = (ulong)System.DateTime.Now.Ticks;
                }
            }
            catch (System.Exception ex)
            {
        ...
        private void ObjectIndexFrameReceived(object state, ReceivedMessageEventArgs args)
        {
            if (sensorData == null)
                return;
            ...
            try
            {
                if (sensorData.objectIndexImage == null || sensorData.objectIndexImage.Length != (args.message.frameWidth * args.message.frameHeight))
                {
                    //rawObjectIndexImage = new byte[args.message.frameWidth * args.message.frameHeight];
                    sensorData.objectIndexImage = new byte[args.message.frameWidth * args.message.frameHeight];
                }
                ...
                lock (objectTrackerLock)
                {
                    rawObjectIndexImage = args.message.frameData;
                    rawObjectIndexTimestamp = args.message.timestamp;
                    //Debug.Log("ReceivedObjectIndexTimestamp: " + rawObjectIndexTimestamp);

                    lastObjectIndexFrameTime = (ulong)System.DateTime.Now.Ticks;
                }
            }
            catch (System.Exception ex)
            {
        ...
        private void Depth2ColorFrameReceived(object state, ReceivedMessageEventArgs args)
        {
            ...
            try
            {
                if (depth2colorImageWidth != args.message.frameWidth || depth2colorImageHeight != args.message.frameHeight)
                {
        ...
        private void Color2InfraredFrameReceived(object state, ReceivedMessageEventArgs args)
        {
            ...
            try
            {
                if (color2infraredImageWidth != args.message.frameWidth || color2infraredImageHeight != args.message.frameHeight)
                {
        ...
        private void Color2ObjectIndexFrameReceived(object state, ReceivedMessageEventArgs args)
        {
            if (sensorData == null)
                return;
            ...
            try
            {
                if (colorCamObjIndexFrame == null || (args.message.frameWidth * args.message.frameHeight) != colorCamObjIndexFrame.Length)
                {
                    colorCamObjIndexFrame = new byte[args.message.frameWidth * args.message.frameHeight];
                }
                ...
                {
                    KinectInterop.CopyBytes(args.message.frameData, sizeof(byte), colorCamObjIndexFrame, sizeof(byte));
                    lastColorCamObjIndexFrameTime = args.message.timestamp;
                    //Debug.Log("ReceivedColorCamObjIndexTimestamp: " + lastColorCamObjIndexFrameTime);

                    lastColor2objIndexFrameTime = (ulong)System.DateTime.Now.Ticks;
                }
            }
            catch (System.Exception ex)
            {
        ...
        private string GetMissingSyncFramesInfo(ulong frameTime, Dictionary<NetMessageType, NetMessageData> dictTimeFrames)
        {
            ...
            if (isFrameSyncNeeded && colorFrameClient != null && !dictTimeFrames.ContainsKey(NetMessageType.Color))
                sbBuf.Append("color ");
            if (isSyncDepthAndColor && depthFrameClient != null && !dictTimeFrames.ContainsKey(NetMessageType.Depth))
                sbBuf.Append("depth ");
            if (isSyncDepthAndColor && infraredFrameClient != null && !dictTimeFrames.ContainsKey(NetMessageType.Infrared))
                sbBuf.Append("ir ");
            ...
            if (isSyncBodyAndDepth && bodyIndexFrameClient != null && !dictTimeFrames.ContainsKey(NetMessageType.BodyIndex))
                sbBuf.Append("bi ");
            if (isSyncBodyAndDepth && objectDataFrameClient != null && !dictTimeFrames.ContainsKey(NetMessageType.ObjectData))
                sbBuf.Append("obj ");
            if (isSyncBodyAndDepth && objectIndexFrameClient != null && !dictTimeFrames.ContainsKey(NetMessageType.ObjectIndex))
                sbBuf.Append("oi ");
            ...
            if (isSyncBodyAndDepth && color2bodyIndexFrameClient != null && !dictTimeFrames.ContainsKey(NetMessageType.Color2BodyIndex))
                sbBuf.Append("c2bi ");
            if (isSyncBodyAndDepth && color2objIndexFrameClient != null && !dictTimeFrames.ContainsKey(NetMessageType.Color2ObjectIndex))
                sbBuf.Append("c2oi ");
        ...
        private void SyncCheckIfAllFramesReady(ulong frameTime, Dictionary<NetMessageType, NetMessageData> dictTimeFrames)
        {
            ...
            if (isFrameSyncNeeded && colorFrameClient != null)
                bAllFramesPresent &= dictTimeFrames.ContainsKey(NetMessageType.Color);
            if (isSyncDepthAndColor && depthFrameClient != null)
                bAllFramesPresent &= dictTimeFrames.ContainsKey(NetMessageType.Depth);
            if (isSyncDepthAndColor && infraredFrameClient != null)
            ...
            if (isSyncBodyAndDepth && objectDataFrameClient != null)
                bAllFramesPresent &= dictTimeFrames.ContainsKey(NetMessageType.ObjectData);
            if (isSyncBodyAndDepth && objectIndexFrameClient != null)
                bAllFramesPresent &= dictTimeFrames.ContainsKey(NetMessageType.ObjectIndex);
            ...
            if (isSyncBodyAndDepth && color2objIndexFrameClient != null)
                bAllFramesPresent &= dictTimeFrames.ContainsKey(NetMessageType.Color2ObjectIndex);
        ...
        private void SyncSendSynchedFrames(ulong frameTime, Dictionary<NetMessageType, NetMessageData> dictTimeFrames)
        {
            ...
            foreach (NetMessageData message in dictTimeFrames.Values)
            {
                ...
                switch (message.msgType)
                {
                    ...
                    case NetMessageType.ObjectData:
                        ObjectDataFrameReceived(this, args);
                        break;

                    case NetMessageType.ObjectIndex:
                        ObjectIndexFrameReceived(this, args);
                        break;
                    ...
                    case NetMessageType.Color2ObjectIndex:
                        Color2ObjectIndexFrameReceived(this, args);
                        break;
        ...
        private ulong GetLatestTimestamp()
        {
            ...
            if (lastObjectDataFrameTime != 0 && lastObjectDataFrameTime > maxFrameTime)
                maxFrameTime = lastObjectDataFrameTime;
            if (lastObjectIndexFrameTime != 0 && lastObjectIndexFrameTime > maxFrameTime)
                maxFrameTime = lastObjectIndexFrameTime;
            ...
            if (lastColor2objIndexFrameTime != 0 && lastColor2objIndexFrameTime > maxFrameTime)
                maxFrameTime = lastColor2objIndexFrameTime;
