...
namespace com.rfilkov.kinect
{
    ...
    public class KinectInterop
    {
        ...
        internal const int THREAD_SLEEP_TIME_MS = 1;
        ...
        public static class Constants
        {
            ...
            public const int MaxObjectCount = 50;
        ...
        public struct JointData
        {
            ...
            public void CopyTo(ref JointData toJoint)
            {
                ...
                //toJoint.imagePos.Set(imagePos.x, imagePos.y);
            ...
            public void AverageTo(ref JointData toJoint)
            {
                ...
                //toJoint.imagePos = (imagePos + toJoint.imagePos) * 0.5f;
        ...
        public class ObjectData
        {
            ...
            public override string ToString()
            {
                return $"{trackingID}/{objType} - oi: {objIndex}, score: {score:F3}, pos: {position:F2}\ncenter: {center2d:F2}; size: {size2d:F2}";
            }

            public void CopyTo(ref ObjectData toObj)
            {
                toObj.trackingID = trackingID;
                toObj.objIndex = objIndex;
                toObj.isTracked = isTracked;

                toObj.objType = objType;
                toObj.score = score;

                toObj.kinectPos = kinectPos;
                toObj.position = position;

                toObj.center2d = center2d;
                toObj.size2d = size2d;

                toObj.lastTimestamp = lastTimestamp;
            }
        }
        ...
        public class SensorDeviceInfo
        {
            ...
            public bool isDepthCam;

            public FrameSource sensorCaps;

            public override string ToString()
            {
                string outName = !string.IsNullOrEmpty(sensorName) ? sensorName : "-";
                return $"{outName}, id: {sensorId}, devIndex: {deviceIndex}, frFacing: {isFrontFacing}, depth: {isDepthCam}";
            }
        }
        ...
        // current intr/extr file version
        public const int SIE_VERSION = 2;
        ...
        public class SensorData
        {
            ...
            public byte[] objectIndexImage;
            public ulong lastObjectIndexFrameTime = 0;

            public uint trackedObjectsCount = 0;
            public ObjectData[] alTrackedObjects;
            public ulong lastObjectFrameTime = 0;
            ...
            //public Texture2D depthImageBuffer;
            //public Texture2D depthHistBuffer;
            ...
            //public Texture2D bodyIndexBuffer;
            public ComputeBuffer bodyIndexBuffer;
            //public ComputeBuffer bodyHistBuffer;
            public ulong lastBodyImageTime = 0;

            public RenderTexture objectImageTexture;
            public Material objectImageMaterial;
            //public Texture2D objectIndexBuffer;
            public ComputeBuffer objectIndexBuffer;
            //public ComputeBuffer objectHistBuffer;
            public ulong lastObjectImageTime = 0;
...
//            public Vector2[] color2DepthFrame = null;
            ...
            public byte[] colorCamObjectIndexImage;
            public ulong lastColorCamObjectIndexFrameTime = 0;
            ...
            public ComputeBuffer colorObjectIndexBuffer;
            public RenderTexture colorObjectIndexTexture;
            public ulong lastColorObjectIndexBufferTime = 0;
            //public ulong usedColorObjectIndexBufferTime = 0;
        ...
        // minimum time between frames and inferences, in ticks.
        public const ulong MIN_TIME_BETWEEN_IN_FRAMES = 10000000 / 60;
        public const ulong MIN_TIME_BETWEEN_INF = 10000000 / 30;
        public const long MIN_TIME_BETWEEN_OUT_FRAMES = 10000000 / 50;
        public const int MAX_LASTINF_QUEUE_LENGTH = 5;
        ...
        // whether the app supports compute shaders or not
        private static bool isSupportComputeShaders = false;


        // sets the graphics shader level
        public static void SetGraphicsShaderLevel(int shaderLevel, bool supportsComputeShaders)
        {
            graphicsShaderLevel = shaderLevel;
            isSupportComputeShaders = supportsComputeShaders;  // false;  // 
            persistentDataPath = Application.persistentDataPath;
            //Debug.Log($"shaderLevel: {graphicsShaderLevel}, computeShaders: {isSupportComputeShaders}\ndataPath: {persistentDataPath}");

            //Application.targetFrameRate = 120;  // (int)Screen.currentResolution.refreshRateRatio.value;
            //Debug.Log($"Setting target frame rate to {Application.targetFrameRate}");

            //if(Application.isMobilePlatform && Application.targetFrameRate <= 0)
            //{
        ...
        // checks whether the system supports compute shaders
        public static bool IsSupportsComputeShaders()
        {
            return isSupportComputeShaders;
        }
...
//#endif
        ...
        // creates new 2d texture with the given parameters
        public static Texture2D CreateTexture2D(Texture2D currentTex, int width, int height, TextureFormat format = TextureFormat.ARGB32)
        {
            if (currentTex != null)
            {
                KinectInterop.Destroy(currentTex);
            }

            Texture2D tex2d = new Texture2D(width, height, format, false);
            return tex2d;
        }
        ...
        public static void InitSensorData(SensorData sensorData, KinectManager kinectManager)
        {
            ...
            //// init object texture
            //if (IsDirectX11Available() && sensorData.objectIndexImage != null &&
            //    (kinectManager.getObjectFrames == KinectManager.ObjectTrackingType.ObjectTexture))  // ||
            //    //kinectManager.getObjectFrames == KinectManager.ObjectTrackingType.SegmentationTexture))
            //{
            //    Shader objectImageShader = null;

            //    switch (kinectManager.getObjectFrames)
            //    {
            //        case KinectManager.ObjectTrackingType.ObjectTexture:
            //            objectImageShader = Shader.Find("Kinect/UserBodyImageShader");
            //            break;

            //        //case KinectManager.ObjectTrackingType.SegmentationTexture:
            //        //    objectImageShader = Shader.Find("Kinect/UserHistImageShader");
            //        //    break;
            //    }

            //    if (objectImageShader != null)
            //    {
            ...
            //        {
            //            sensorData.objectImageTexture = CreateRenderTexture(sensorData.objectImageTexture, sensorData.depthImageWidth, sensorData.depthImageHeight);
            //        }

            //        sensorData.objectImageMaterial = new Material(objectImageShader);

            //        //if (sensorData.objectIndexBuffer == null)
            //        //{
            //        //    int objIndexBufferLength = sensorData.depthImageWidth * sensorData.depthImageHeight >> 2;
            //        //    sensorData.objectIndexBuffer = CreateComputeBuffer(sensorData.objectIndexBuffer, objIndexBufferLength, sizeof(uint));
            //        //}
            ...
            //        //{
            //        //    sensorData.objectHistBuffer = CreateComputeBuffer(sensorData.objectHistBuffer, DepthSensorBase.MAX_DEPTH_DISTANCE_MM + 1, sizeof(int));
            //        //}
            //    }
            //}
        ...
        public static void CloseSensor(SensorData sensorData)
        {
            ...
            if (sensorData.depthHistBuffer != null)
            {
                //KinectInterop.Destroy(sensorData.depthHistBuffer);
            ...
            if (sensorData.bodyIndexBuffer != null)
            {
                //KinectInterop.Destroy(sensorData.bodyIndexBuffer);
            ...
            if (sensorData.objectImageTexture)
            {
                sensorData.objectImageTexture.Release();
                sensorData.objectImageTexture = null;
            }

            if (sensorData.objectIndexBuffer != null)
            {
                //KinectInterop.Destroy(sensorData.objectIndexBuffer);
                sensorData.objectIndexBuffer.Release();
                sensorData.objectIndexBuffer.Dispose();
                sensorData.objectIndexBuffer = null;
            }

            //if (sensorData.objectHistBuffer != null)
            //{
            //    sensorData.objectHistBuffer.Release();
            //    sensorData.objectHistBuffer.Dispose();
            //    sensorData.objectHistBuffer = null;
            //}
        }
        ...
        public static void CopyBytes(IntPtr srcPtr, IntPtr dstPtr, int byteCount)
        {
            ...
            unsafe
            {
            Unity.Collections.LowLevel.Unsafe.UnsafeUtility.MemCpy((void*)dstPtr, (void*)srcPtr, byteCount);
            }
#else
        ...
        public static void CopyBytes(Array srcArray, int copyElemCount, int srcElemSize, IntPtr dstPtr, int dstBlockSize)
        {
            ...
            if (copyBytesCount > 0)
            {
                ...
                unsafe
                {
                    Unity.Collections.LowLevel.Unsafe.UnsafeUtility.MemCpy((void*)dstPtr, (void*)pSrcData.AddrOfPinnedObject(), copyBytesCount);
                }
#else
        ...
        public static void CopyBytes(IntPtr srcPtr, int srcBlockSize, Array dstArray, int copyElemCount, int dstElemSize)
        {
            ...
            if (copyBytesCount > 0)
            {
                ...
                unsafe
                {
                    Unity.Collections.LowLevel.Unsafe.UnsafeUtility.MemCpy((void*)pDstData.AddrOfPinnedObject(), (void*)srcPtr, copyBytesCount);
                }
#else
        ...
        public static void CopyBytes(Array srcArray, int srcElemSize, Array dstArray, int dstElemSize)
        {
            ...
            if (copyBytesCount > 0)
            {
                ...
                unsafe
                {
                    Unity.Collections.LowLevel.Unsafe.UnsafeUtility.MemCpy((void*)pDstData.AddrOfPinnedObject(), (void*)pSrcData.AddrOfPinnedObject(), copyBytesCount);
                }
#else
        ...
        // sets compute buffer data
        public static void SetTextureData(Texture2D tex, Array data, int elemCount, int elemSize)
        {
            var pData = GCHandle.Alloc(data, GCHandleType.Pinned);
            tex.LoadRawTextureData(pData.AddrOfPinnedObject(), elemCount * elemSize);
            tex.Apply();
            pData.Free();
        }
        ...
        // converts current object frame to a single csv line. returns empty string if there is no object data
        public static string GetObjectFrameAsCsv(ref ObjectData[] objData, uint objCount, ulong liRelTime, Vector3 spaceScale, ref float fUnityTime, char delimiter)
        {
            ...
            sbBuf.Append("k4o").Append(delimiter);
            sbBuf.Append(liRelTime).Append(delimiter);

            fUnityTime = Time.time;
            sbBuf.Append(objCount).Append(delimiter);
            ...
            {
                sbBuf.Append(objData[i].isTracked ? "1" : "0").Append(delimiter);

                if (objData[i].isTracked)
                {
                    // add information about the tracked object
                    sbBuf.Append(objData[i].trackingID).Append(delimiter);

                    sbBuf.Append(objData[i].objIndex).Append(delimiter);
                    sbBuf.Append((int)objData[i].objType).Append(delimiter);
                    sbBuf.AppendFormat(invCulture, "{0:F3}", objData[i].score).Append(delimiter);

                    Vector3 objPos = objData[i].kinectPos;
                    sbBuf.AppendFormat(invCulture, "{0:F3}", objPos.x).Append(delimiter);
                    sbBuf.AppendFormat(invCulture, "{0:F3}", objPos.y).Append(delimiter);
                    sbBuf.AppendFormat(invCulture, "{0:F3}", objPos.z).Append(delimiter);

                    Vector2 objCenter = objData[i].center2d;
                    sbBuf.AppendFormat(invCulture, "{0:F3}", objCenter.x).Append(delimiter);
                    sbBuf.AppendFormat(invCulture, "{0:F3}", objCenter.y).Append(delimiter);

                    Vector2 objSize = objData[i].size2d;
                    sbBuf.AppendFormat(invCulture, "{0:F3}", objSize.x).Append(delimiter);
                    sbBuf.AppendFormat(invCulture, "{0:F3}", objSize.y).Append(delimiter);

                    //sbBuf.Append((int)objData[i].lastTimestamp).Append(delimiter);
                }
            }
        ...
        public static uint SetObjectFrameFromCsv(string sCsvLine, string sDelim, SensorData sensorData, ref ObjectData[] objData,
            ref Matrix4x4 sensorToWorld, bool bIgnoreZCoords, out ulong liRelTime)
        {
            ...
            // object count
            int.TryParse(alCsvParts[2], out int objCount);

            // space scale
            int.TryParse(alCsvParts[3], out int iSX);
            int.TryParse(alCsvParts[4], out int iSY);
            int.TryParse(alCsvParts[5], out int iSZ);
            ...
            if (alCsvParts[0] != "k4o" || objCount == 0 || liRelTime == 0)
                return 0;

            if (objData.Length < objCount)
            {
                Array.Resize<ObjectData>(ref objData, objCount);
                for (int i = 0; i < objCount; i++)
                {
                    objData[i] = new ObjectData();
                }
            }
            ...
            for (int i = 0; i < objCount; i++)
            {
                ...
                short.TryParse(alCsvParts[iIndex], out isTracked);
                iIndex++;

                if (isTracked != 0 && alCsvParts.Length >= (iIndex + 1))
                {
                    ulong.TryParse(alCsvParts[iIndex], out trackingID);
                    iIndex++;
                ...
                objData[i].isTracked = (isTracked != 0);
                objData[i].trackingID = trackingID;

                if (isTracked != 0)
                {
                    if (alCsvParts.Length >= (iIndex + 3))
                    {
                        int.TryParse(alCsvParts[iIndex], out int objIndex);
                        iIndex++;

                        int.TryParse(alCsvParts[iIndex], out int objType);
                        iIndex++;

                        float.TryParse(alCsvParts[iIndex], numFloat, invCulture, out float score);
                        iIndex++;

                        objData[i].objIndex = objIndex;
                        objData[i].objType = (ObjectType)objType;
                        objData[i].score = score;

                        if (alCsvParts.Length >= (iIndex + 7))
                        {
                            ...
                            objData[i].kinectPos = new Vector3(x, y, z);

                            float.TryParse(alCsvParts[iIndex], numFloat, invCulture, out x);
                            float.TryParse(alCsvParts[iIndex + 1], numFloat, invCulture, out y);
                            iIndex += 2;

                            objData[i].center2d = new Vector2(x, y);

                            float.TryParse(alCsvParts[iIndex], numFloat, invCulture, out x);
                            float.TryParse(alCsvParts[iIndex + 1], numFloat, invCulture, out y);
                            iIndex += 2;

                            objData[i].size2d = new Vector2(x, y);
                        }
                        else
                        {
                            objData[i].kinectPos = Vector3.zero;
                            objData[i].center2d = Vector2.zero;
                            objData[i].size2d = Vector2.zero;
                        }

                        objData[i].position = sensorToWorld.MultiplyPoint3x4(new Vector3(objData[i].kinectPos.x * sensorSpaceScale.x, objData[i].kinectPos.y * sensorSpaceScale.y, objData[i].kinectPos.z));
                        objData[i].lastTimestamp = liRelTime;
                    }

                }
            }

            return (uint)objCount;
        }
            ...
            ref Matrix4x4 sensorToWorld, bool bIgnoreZCoords, out ulong liRelTime)
        {
            ...
            for (int i = 0; i < bodyCount; i++)
            {
                ...
                bodyData[i].bodyTimestamp = liRelTime;
...
//#endif
                ...
                string filePath = string.Format("{0}ST-{1}-{2}-{3}-{4}-{5}-v{6}.bin", saveFolder, sensorPlatform.ToString(), sensorId, camType, frameWidth, frameHeight, SIE_VERSION);
                if (File.Exists(filePath))
                {
                    ...
                    //Debug.Log($"Loaded space table: {filePath}");
                }
            }
...
//#endif
            ...
            //Debug.Log($"Saved space table: {filePath}");
        }

        // checks if the intrinsics & extrinsics file exists or not.
        public static bool IsSensorIntrExtrExist(SensorData sensorData)
        {
            if (sensorData == null)
                return false;
...
//#endif
            ...
            string filePath = string.Format("{0}IE-{1}-{2}-{3}-{4}-{5}-{6}-v{7}.json", saveFolder, sensorData.sensorIntPlatform.ToString(), sensorData.sensorId,
                sensorData.depthImageWidth, sensorData.depthImageHeight, sensorData.colorImageWidth, sensorData.colorImageHeight, SIE_VERSION);
...
//#endif
            ...
            if (File.Exists(filePath))
            {
                ...
                if (sie != null && sie.version == SIE_VERSION)
                {
                    ...
                    //Debug.Log($"Loaded sensor intr-extr: {filePath}");
                    return true;
                }
            }
...
//#endif
            ...
            //Debug.Log($"Saved sensor intr-extr: {filePath}");
        }
        ...
        // checks if the file exists - returns true if the file exists, false otherwise
...
//#endif
        ...
        public static void TransformTexture(Texture srcTex, RenderTexture tgtTex, int rotationAngle = 0, bool flipX = false, bool flipY = false, bool keepST = false)
        {
            ...
            if (isPortraitMode)
            {
                ...
                texST = !keepST ? GetTexST(tgtAspect, srcAspect) : new Vector4(1f, 1f, 0f, 0f);
            }
            else
            {
                transMat = GetTransformMat(rotationAngle, flipX, flipY);
                texST = !keepST ? GetTexST(srcAspect, tgtAspect) : new Vector4(1f, 1f, 0f, 0f);
            }
        ...
        // returns mean and standard deviation of a list of values
        public static float GetStandardDev(float[] list, out float listMean)
        {
            listMean = 0.0f;
            if (list == null || list.Length == 0)
                return 0f;
            ...
            listMean /= list.Length;

            float listVar = 0.0f;
            foreach (float value in list)
            {
                listVar += (value - listMean) * (value - listMean);
            }

            listVar /= list.Length;

            return Mathf.Sqrt(listVar);
        }

        // smooths value based on its history
        public static float SmoothValue(float[] hist)
        {
            float sd = GetStandardDev(hist, out float avg);
            ...
            {
                float current = hist[i];

                if (current >= (avg - sd) && current <= (avg + sd))
                {
                    result += current;
                    count++;
                }
        ...
        // smooths value based on its history
        public static bool IsValueInRange(float val, float[] hist)
        {
            float sd = GetStandardDev(hist, out float avg);
        ...
        private static float OneEuroAlpha(float cutoff, float t_e)
        {
            float r = 2f * Mathf.PI * cutoff * t_e;
            return r / (r + 1);
        }

        // filter params consists of (x: beta, y: fcmin, z: dtime)
        public static float OneEuroStepDX(float x, float p_x, float p_dx, Vector3 filterParams)
        {
            float dx = filterParams.z > 0f ? (x - p_x) / filterParams.z : p_dx;
            return Mathf.Lerp(p_dx, dx, OneEuroAlpha(1f, filterParams.z));
        }

        public static float OneEuroStepX(float x, float p_x, float dx, Vector3 filterParams)
        {
            float cutoff = filterParams.y + filterParams.x * Mathf.Abs(dx);
            return Mathf.Lerp(p_x, x, OneEuroAlpha(cutoff, filterParams.z));
        }

        // filter params consists of (x: beta, y: fcmin, z: dtime)
        public static Vector2 OneEuroStepDX(Vector2 x, Vector2 p_x, Vector2 p_dx, Vector3 filterParams)
        {
            Vector2 dx = filterParams.z > 0f ? (x - p_x) / filterParams.z : p_dx;
            return Vector2.Lerp(p_dx, dx, OneEuroAlpha(1f, filterParams.z));
        }

        public static Vector2 OneEuroStepX(Vector2 x, Vector2 p_x, Vector2 dx, Vector3 filterParams)
        {
            float cutoff = filterParams.y + filterParams.x * dx.magnitude;
            return Vector2.Lerp(p_x, x, OneEuroAlpha(cutoff, filterParams.z));
        }
        ...
        // rotation
        public static Matrix4x4 MakeRotationMatrixZ(float x)
        {
        ...
        public static Matrix4x4 MakeRotationMatrixX(float x)
        {
            Matrix4x4 mat = Matrix4x4.identity;
            mat[1, 1] = Mathf.Cos(x);
            mat[1, 2] = -Mathf.Sin(x);
