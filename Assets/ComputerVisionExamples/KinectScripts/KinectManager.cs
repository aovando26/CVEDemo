...
namespace com.rfilkov.kinect
{
    ...
    public class KinectManager : MonoBehaviour
    {
        ...
        [Tooltip("Whether or not to get object frames from the object tracker.")]
        public ObjectTrackingType getObjectFrames = ObjectTrackingType.None;
        public enum ObjectTrackingType : int { None = 0, ObjectAndObjectIndexData = 1, ObjectTexture = 2, /**SegmentationTexture = 3,*/ ObjectDataOnly = 4, ObjectIndexDataOnly = 5 }
        ...
        protected float waitTimeBeforeRemove = 0f;  // 0.25f;
        ...
        [Tooltip("Object manager, used to track the objects. KM creates one, if not set.")]
        public KinectObjectManager objectManager;
        ...
        public enum DisplayImageType : int
        {
            ...
            ObjectImageS0 = 0x201, ObjectImageS1 = 0x202, ObjectImageS2 = 0x203
        }
        ...
        // object frame data
        protected ulong lastObjectFrameTime = 0;
        protected uint trackedObjectsCount = 0;
        protected KinectInterop.ObjectData[] alTrackedObjects = new KinectInterop.ObjectData[0];

        protected long lastObjectFrameTicks = 0;
        protected long prevObjectFrameTicks = 0;
        ...
        /// <summary>
        /// Gets the min player-index of the currently tracked users.
        /// </summary>
        /// <returns>The min player-index of the tracked users.</returns>
        public int GetMinUserIndex()
        {
            int minIndex = -1;
        ...
        /// <summary>
        /// Gets all user body data (do not change the data in the structure)
        /// </summary>
        ...
        /// <summary>
        /// Gets the user body data (do not change the data in the structure).
        /// </summary>
        /// <returns>The user body data.</returns>
        /// <param name="index">Body index</param>
        public KinectInterop.BodyData GetUserBodyData(int index)
        {
        ...
        /// </summary>
        /// <returns>The user body data.</returns>
        /// <param name="objId">User ID</param>
        public KinectInterop.BodyData GetUserBodyData(ulong userId)
        {
            if (userManager.dictUserIdToIndex.ContainsKey(userId))
            {
                int index = userManager.dictUserIdToIndex[userId];
        ...
        private static int[] UBB_Joints = {
            ...
            (int)KinectInterop.JointType.ShoulderLeft, (int)KinectInterop.JointType.ElbowLeft, (int)KinectInterop.JointType.WristLeft, (int)KinectInterop.JointType.HandLeft, (int)KinectInterop.JointType.HandtipLeft,  // left
            (int)KinectInterop.JointType.ShoulderRight, (int)KinectInterop.JointType.ElbowRight, (int)KinectInterop.JointType.WristRight, (int)KinectInterop.JointType.HandRight, (int)KinectInterop.JointType.HandtipRight,  // right
            ...
            out Vector3 posMin, out Vector3 posMax)
        {
            ...
            for (int i = 0; i < iCount; i++)
            {
                ...
                //if (IsJointTracked(userId, j))
                {
                    ...
                    //Debug.Log($"  user {userId} {(KinectInterop.JointType)j}, pos: {jPos:F2}");
        ...
        /// <summary>
        /// Gets the last object frame time, as returned by the sensor.
        /// </summary>
        /// <param name="sensorIndex">The sensor index.</param>
        /// <returns>The object frame time.</returns>
        public ulong GetObjectFrameTime(int sensorIndex)
        {
            KinectInterop.SensorData sensorData = GetSensorData(sensorIndex);
            return sensorData != null ? sensorData.lastObjectFrameTime : 0;
        }

        /// <summary>
        /// Gets the last object-index frame time, as returned by the sensor.
        /// </summary>
        /// <param name="sensorIndex">The sensor index.</param>
        /// <returns>The object-index frame time.</returns>
        public ulong GetObjectIndexFrameTime(int sensorIndex)
        {
            KinectInterop.SensorData sensorData = GetSensorData(sensorIndex);
            return sensorData != null ? sensorData.lastObjectIndexFrameTime : 0;
        }

        /// <summary>
        /// Gets the object-image texture.
        /// </summary>
        /// <returns>The object-image texture.</returns>
        public Texture GetObjectImageTex()
        {
            return GetObjectImageTex(btSensorIndex);
        }
        ...
        /// <param name="sensorIndex">The sensor index.</param>
        /// <returns>The object-image texture.</returns>
        public Texture GetObjectImageTex(int sensorIndex)
        {
            KinectInterop.SensorData sensorData = GetSensorData(sensorIndex);
            if (sensorData != null)
            {
                return sensorData.objectImageTexture != null ? sensorData.objectImageTexture : sensorData.depthImageTexture;
            }
        ...
        /// <summary>
        /// Determines whether the object with the specified index is currently detected by the sensor
        /// </summary>
        /// <returns><c>true</c> if the object is detected; otherwise, <c>false</c>.</returns>
        /// <param name="i">Object index.</param>
        public bool IsObjectDetected(int i)
        {
            if (i >= 0 && i < KinectInterop.Constants.MaxObjectCount)
            {
                return (objectManager.aObjectIndexIds[i] != 0);
            }
        ...
        ///// <summary>
        ///// Determines whether the object with the specified objId is in the list of tracked object or not.
        ///// </summary>
        ///// <returns><c>true</c> if the object with the specified objId is tracked; otherwise, <c>false</c>.</returns>
        ///// <param name="userId">Object ID</param>
        //public bool IsObjectTracked(ulong objId)
        //{
        //    return objectManager.dictObjectIdToIndex.ContainsKey(objId);
        //}
        ...
        //{
        //    return objectManager.alObjectIds.Count;
        //}

        /// <summary>
        /// Gets the IDs of all currently tracked objects.
        /// </summary>
        /// <returns>The list of all currently tracked objects.</returns>
        public List<ulong> GetAllObjectIds()
        {
            return new List<ulong>(objectManager.alObjectIds);
        }

        /// <summary>
        /// Gets the min object-index of the currently tracked objects.
        /// </summary>
        /// <returns>The max object-index of the tracked objects.</returns>
        public int GetMinObjectIndex()
        {
            int minIndex = -1;

            for (int i = 0; i < KinectInterop.Constants.MaxObjectCount; i++)
            {
                if (objectManager.aObjectIndexIds[i] != 0)
                {
                    minIndex = i;
                    break;
                }
        ...
        /// <summary>
        /// Gets the max object-index of the currently tracked objects.
        /// </summary>
        /// <returns>The max object-index of the tracked objects.</returns>
        public int GetMaxObjectIndex()
        {
            ...
            for (int i = KinectInterop.Constants.MaxObjectCount - 1; i >= 0; i--)
            {
                if (objectManager.aObjectIndexIds[i] != 0)
                {
                    maxIndex = i;
        ...
        /// <summary>
        /// Gets the object indices of all currently tracked objects.
        /// </summary>
        /// <returns>The list of object-indices of all tracked objects.</returns>
        public List<int> GetAllObjectIndices()
        {
            List<int> alIndices = new List<int>();
        ...
        /// <summary>
        /// Gets the object ID by the specified object index.
        /// </summary>
        /// <returns>The object ID by index.</returns>
        /// <param name="i">The user index.</param>
        public ulong GetObjectIdByIndex(int i)
        {
            if (i >= 0 && i < KinectInterop.Constants.MaxObjectCount)
            {
                return objectManager.aObjectIndexIds[i];
            }
        ...
        /// <summary>
        /// Gets the object index by the specified object ID.
        /// </summary>
        /// <returns>The object index by object ID.</returns>
        /// <param name="objId">Object ID</param>
        public int GetObjectIndexById(ulong objId)
        {
            if (objId == 0)
                return -1;

            for (int i = 0; i < objectManager.aObjectIndexIds.Length; i++)
            {
                if (objectManager.aObjectIndexIds[i] == objId)
                {
                    return i;
                }
            }
        ...
        /// <summary>
        /// Gets the object tracking index by the specified object ID, or -1 if the object ID does not exist.
        /// </summary>
        /// <returns>Object tracking index by object ID.</returns>
        /// <param name="objId">Object ID</param>
        public int GetTrackingIndexByObjectId(ulong objId)
        {
            if (objectManager.dictObjectIdToIndex.ContainsKey(objId))
            {
                int index = objectManager.dictObjectIdToIndex[objId];

                if (index >= 0 && index < trackedObjectsCount && alTrackedObjects[index].isTracked)
                {
                    int objIndex = alTrackedObjects[index].objIndex;
                    return objIndex;
                }
            }
        ...
        /// <summary>
        /// Gets the list of tracked object indices.
        /// </summary>
        /// <returns>The list of object indices.</returns>
        public List<int> GetTrackedObjectIndices()
        {
            List<int> alObjectIndices = new List<int>(objectManager.dictObjectIdToIndex.Values);
            return alObjectIndices;
        }

        /// <summary>
        /// Gets the last object frame timestamp.
        /// </summary>
        /// <returns>The last object frame timestamp.</returns>
        public ulong GetObjectFrameTimestamp()
        {
            return lastObjectFrameTime;
        }

        /// <summary>
        /// Gets all object data (do not change the data in the structure).
        /// </summary>
        /// <returns>Array of the available object data</returns>
        internal KinectInterop.ObjectData[] GetAllObjectData()
        {
            return alTrackedObjects;
        }

        /// <summary>
        /// Gets the object data, or new data if not found (do not change the data in the structure).
        /// </summary>
        /// <returns>The object data.</returns>
        /// <param name="index">Object (tracking) index</param>
        public KinectInterop.ObjectData GetObjectData(int index)
        {
            if (index >= 0 && index < trackedObjectsCount)
            {
                return alTrackedObjects[index];
            }

            return new KinectInterop.ObjectData();
        }

        /// <summary>
        /// Gets the object data (do not change the data in the structure).
        /// </summary>
        /// <returns>The object data.</returns>
        /// <param name="objId">Object ID</param>
        public KinectInterop.ObjectData GetObjectData(ulong objId)
        {
            if (objectManager.dictObjectIdToIndex.ContainsKey(objId))
            {
                int index = objectManager.dictObjectIdToIndex[objId];
        ...
        /// <summary>
        /// Determines whether the given object is tracked or not.
        /// </summary>
        /// <returns><c>true</c> if the object is tracked; otherwise, <c>false</c>.</returns>
        /// <param name="objId">User ID</param>
        public bool IsObjectTracked(ulong objId)
        {
            if (objectManager.dictObjectIdToIndex.ContainsKey(objId))
            {
                ...
                if (index >= 0 && index < trackedObjectsCount)
                {
                    return alTrackedObjects[index].isTracked;
                }
            }
        ...
        /// <summary>
        /// Gets object timestamp.
        /// </summary>
        /// <returns>Object timestamp.</returns>
        /// <param name="objId">Object ID</param>
        public ulong GetObjectTimestamp(ulong objId)
        {
            if (objectManager.dictObjectIdToIndex.ContainsKey(objId))
            {
                int index = objectManager.dictObjectIdToIndex[objId];
                ...
                if (index >= 0 && index < trackedObjectsCount)
                {
                    return alTrackedObjects[index].lastTimestamp;
                }
            }
        ...
        /// <summary>
        /// Gets the object position in Kinect coordinate system, in meters.
        /// </summary>
        /// <returns>The object kinect position.</returns>
        /// <param name="objId">Object ID</param>
        /// <param name="applyDepthScale">Whether to apply the sensor space scale or not</param>
        public Vector3 GetObjectKinectPosition(ulong objId, bool applySpaceScale)
        {
            if (objectManager.dictObjectIdToIndex.ContainsKey(objId))
            {
                int index = objectManager.dictObjectIdToIndex[objId];
                ...
                if (index >= 0 && index < trackedObjectsCount && alTrackedObjects[index].isTracked)
                {
                    Vector3 objKinectPos = alTrackedObjects[index].kinectPos;

                    if (applySpaceScale && btSensorIndex >= 0 && btSensorIndex < sensorDatas.Count)
                    {
                        Vector3 spaceScale = sensorDatas[btSensorIndex].sensorSpaceScale;
                        return new Vector3(objKinectPos.x * spaceScale.x, objKinectPos.y * spaceScale.y, objKinectPos.z);
                    }
                    else
                    {
                        return objKinectPos;
        ...
        /// <summary>
        /// Gets the object position, relative to the sensor, in meters.
        /// </summary>
        /// <returns>The object position.</returns>
        /// <param name="objId">Object ID</param>
        public Vector3 GetObjectPosition(ulong objId)
        {
            if (objectManager.dictObjectIdToIndex.ContainsKey(objId))
            {
                int index = objectManager.dictObjectIdToIndex[objId];
                ...
                if (index >= 0 && index < trackedObjectsCount && alTrackedObjects[index].isTracked)
                {
                    return alTrackedObjects[index].position;
                }
            }
        ...
        /// <param name="objId">Object ID</param>
        public KinectInterop.ObjectType GetObjectType(ulong objId)
        {
            if (objectManager.dictObjectIdToIndex.ContainsKey(objId))
            {
                int index = objectManager.dictObjectIdToIndex[objId];
                ...
                if (index >= 0 && index < trackedObjectsCount && alTrackedObjects[index].isTracked)
                {
                    return alTrackedObjects[index].objType;
                }
            }

            return KinectInterop.ObjectType.Count;
        }

        /// <summary>
        /// Gets the number of objects, tracked by the sensor.
        /// </summary>
        /// <returns>The objects count.</returns>
        public int GetObjectCount()
        {
            return (int)trackedObjectsCount;
        }

        /// <summary>
        /// Gets the maximum possible number of objects, tracked by the sensor.
        /// </summary>
        /// <returns>The maximum object count.</returns>
        public int GetMaxObjectCount()
        {
            return KinectInterop.Constants.MaxObjectCount;
        }
            ...
            if (userManager.dictUserIdToIndex.ContainsKey(userId) && camera != null)
            {
                ...
                if (index >= 0 && index < trackedBodiesCount && alTrackedBodies[index].bIsTracked)
                {
                    if (joint >= 0 && joint < (int)KinectInterop.JointType.Count)
                    {
                        ...
                        return GetPosDepthOverlay(posJointRaw, sensorIndex, camera, imageRect);
                    }
                }
            }
        ...
        public Vector3 GetPosDepthOverlay(Vector3 posSensorSpace, int sensorIndex, Camera camera, Rect imageRect)
        {
            ...
            if (userManager.dictUserIdToIndex.ContainsKey(userId) && camera != null)
            {
                ...
                if (index >= 0 && index < trackedBodiesCount && alTrackedBodies[index].bIsTracked)
                {
                    if (joint >= 0 && joint < (int)KinectInterop.JointType.Count)
                    {
                        ...
                        return GetPosColorOverlay(posJointRaw, sensorIndex, camera, imageRect);
                    }
                }
            }
        ...
        /// <param name="camera">Camera used to visualize the 3d overlay position</param>
        /// <param name="imageRect">Color image rectangle on the screen</param>
        public Vector3 GetPosColorOverlay(Vector3 posSensorSpace, int sensorIndex, Camera camera, Rect imageRect)
        {
            if (posSensorSpace != Vector3.zero)
            {
                // 3d position to depth
        ...
        /// <param name="joint">User joint</param>
        /// <param name="imageRect">Color image rectangle on the screen</param>
        public Vector3 GetJointPosColorOverlay(ulong userId, KinectInterop.JointType joint, int sensorIndex, Rect imageRect)
        {
        ...
        /// <param name="joint">Joint index</param>
        /// <param name="imageRect">Color image rectangle on the screen</param>
        public Vector3 GetJointPosColorOverlay(ulong userId, int joint, int sensorIndex, Rect imageRect)
        {
            if (userManager.dictUserIdToIndex.ContainsKey(userId))
            {
                int index = userManager.dictUserIdToIndex[userId];
                ...
                if (index >= 0 && index < trackedBodiesCount && alTrackedBodies[index].bIsTracked)
                {
                    if (joint >= 0 && joint < (int)KinectInterop.JointType.Count)
                    {
                        ...
                        return GetPosColorOverlay(posJointRaw, sensorIndex, imageRect);
                    }
                }
            }
        ...
        /// <param name="imageRect">Color image rectangle on the screen</param>
        /// <returns></returns>
        public Vector3 GetPosColorOverlay(Vector3 posSensorSpace, int sensorIndex, Rect imageRect)
        {
            if (posSensorSpace != Vector3.zero)
            {
                // 3d position to depth
                ...
                if (posColor.x != 0f && !float.IsInfinity(posColor.x) && sensorData != null)
                {
                    float xScaled = (float)posColor.x * imageRect.width / sensorData.colorImageWidth;
                    float yScaled = (float)posColor.y * imageRect.height / sensorData.colorImageHeight;
                    ...
                    return new Vector3(xImage, yImage, posSensorSpace.z);
                }
            }
        ...
        public Color[] GetBodyIndexColors()
        {
            ...
            ulong numBodyIndexColors = (ulong)_bodyIndexColors.Length;
            System.Text.StringBuilder sbBuf = new System.Text.StringBuilder();

            foreach (ulong userId in userManager.dictUserIdToIndex.Keys)
            {
                if (userId != 0)
                {
                    int index = userManager.dictUserIdToIndex[userId];
                    ...
                    if (index >= 0 && index < trackedBodiesCount && alTrackedBodies[index].bIsTracked)
                    {
                        ...
                        clrUsers[bi] = (index == 0) ? Color.yellow : _bodyIndexColors[userId % numBodyIndexColors];
                        sbBuf.AppendLine($"body-clr id: {userId}, index: {index}, bi: {bi}, color: {clrUsers[bi]}");
                    }
                }
            }
        ...
        private static readonly Color[] _bodyIndexColors = { Color.red, Color.green, Color.blue, Color.magenta, Color.cyan, Color.gray };
        ...
        /// <summary>
        /// Returns array of colors, one for each object index.
        /// </summary>
        /// <returns>Array of object index colors.</returns>
        public Color[] GetObjectIndexColors()
        {
            if (_initialObjIndexColors == null)
            {
                _initialObjIndexColors = new Color[KinectInterop.Constants.MaxObjectCount];
                for (int i = 0; i < _initialObjIndexColors.Length; i++)
                    _initialObjIndexColors[i] = Color.white;  // showAllowedObjectsOnly ? _bodyColorNone : Color.white;
            }

            KinectInterop.CopyBytes(_initialObjIndexColors, 4 * sizeof(float), clrObjects, 4 * sizeof(float));
            ulong numObjIndexColors = (ulong)_objIndexColors.Length;
            System.Text.StringBuilder sbBuf = new System.Text.StringBuilder();

            foreach(ulong objId in objectManager.dictObjectIdToIndex.Keys)
            {
                if (objId != 0)
                {
                    int index = objectManager.dictObjectIdToIndex[objId];
                    ...
                    if (index >= 0 && index < trackedObjectsCount && alTrackedObjects[index].isTracked)
                    {
                        int oi = alTrackedObjects[index].objIndex;
                        clrObjects[oi] = _objIndexColors[objId % numObjIndexColors];
                        sbBuf.AppendLine($"obj-clr id: {objId}/{alTrackedObjects[index].objType}, index: {index}, oi: {oi}, color: {clrObjects[oi]}");
                    }
                }
            }
        ...
        // object colors
        private static readonly Color[] _objIndexColors = {
            new Color(1f, 0.753f, 0.796f),  // Pink (255, 192, 203)
            new Color(0.596f, 0.984f, 0.596f),  // PaleGreen (152, 251, 152)
            new Color(0.529f, 0.808f, 0.980f),  // LightSkyBlue (135, 206, 250)
            new Color(1f, 0.843f, 0f),  // Gold (255, 215, 0)
            new Color(0.867f, 0.627f, 0.867f),  // Plum (221, 160, 221)
            new Color(1f, 0.894f, 0.769f),  // Bisque (255, 228, 196)
            new Color(0.941f, 0.902f, 0.549f),  // Khaki (240, 230, 140)
            new Color(1f, 0.941f, 0.961f)  // LavenderBlush (255, 240, 245)
        };
        private Color[] clrObjects = new Color[KinectInterop.Constants.MaxObjectCount];

        // initial object-index color array
        private static Color[] _initialObjIndexColors = null;
        ...
        void Awake()
        {
            ...
            KinectInterop.SetGraphicsShaderLevel(SystemInfo.graphicsShaderLevel, SystemInfo.supportsComputeShaders);
            ...
            // object manager by default 
            if (objectManager == null)
            {
                objectManager = gameObject.GetComponent<KinectObjectManager>();

                if (objectManager == null)
                {
                    objectManager = gameObject.AddComponent<KinectObjectManager>();
                }
            }
        ...
        private KinectInterop.FrameSource GetFrameSourceFlags()
        {
            ...
            if (getObjectFrames != ObjectTrackingType.None)
            {
                if (getObjectFrames != ObjectTrackingType.ObjectIndexDataOnly)
                    dwFlags |= KinectInterop.FrameSource.TypeObject;
                if (getObjectFrames != ObjectTrackingType.ObjectDataOnly)
                    dwFlags |= KinectInterop.FrameSource.TypeObjectIndex;
            }
        ...
        private DepthSensorBase CreateSensorInterface(KinectInterop.DepthSensorPlatform sensorType, int sensorIndex)
        {
            DepthSensorBase sensorInt = null;
            string intTypeName = string.Empty;

            GameObject sensorObj = new GameObject(sensorType.ToString() + sensorIndex);
            ...
            switch (sensorType)
            {
                ...
                default:
                    throw new Exception("Unknown S" + sensorIndex + " sensor interface: " + sensorType);
            }

            if(!string.IsNullOrEmpty(intTypeName))
            {
                if (consoleLogMessages)
                    Debug.Log("Creating S" + sensorIndex + ": '" + sensorType + "' sensor interface...");
...
//            }
            ...
            try
            {
                ...
                if (useMultiCamConfig)
                {
                    ...
                    if (!string.IsNullOrEmpty(multiCamJson))
                    {
                        ...
                        // remove current sensor interfaces
                        ...
                        for (int i = 0; i < multiCamConfig.camPose.Length; i++)
                        {
                            ...
                            int sensorIndex = i;  // multiCamConfig.camPose[i].sensorIndex;

                            DepthSensorBase sensorInt = CreateSensorInterface(sensorType, sensorIndex);
                            if (sensorInt != null)
                            {
                                // set settings
        ...
        void Update()
        {
            ...
            {
                //_frameNumber++;
                //Debug.LogWarning($"FRAME <<{_frameNumber}>>");
                ...
                for (int i = 0; i < sensorDatas.Count; i++)
                {
                    ...
                    if (KinectInterop.UpdateSensorData(sensorData, this, isPlayModeEnabled))
                    {
                        ...
                        UpdateTrackedObjects(i, sensorData);
                    }
                }
        ...
        void OnGUI()
        {
            ...
            for (int i = 0; i < displayImages.Count; i++)
            {
                ...
                switch (imageType)
                {
                    ...
                    case DisplayImageType.ObjectImageS0:
                    case DisplayImageType.ObjectImageS1:
                    case DisplayImageType.ObjectImageS2:
                        si = imageType == DisplayImageType.ObjectImageS0 ? 0 : imageType == DisplayImageType.ObjectImageS1 ? 1 : imageType == DisplayImageType.ObjectImageS2 ? 2 : -1;
                        if (si >= 0 && si < sensorDatas.Count)
                        {
                            KinectInterop.SensorData sensorData = sensorDatas[si];
                            imageScale = sensorData.depthImageScale;
                            imageTex = sensorData.objectImageTexture ? sensorData.objectImageTexture : sensorData.depthImageTexture;
                        }
                        break;
                }
        ...
        // updates the global list of tracked objects
        protected void UpdateTrackedObjects(int sensorIndex, KinectInterop.SensorData sensorData)
        {
            if (sensorDatas.Count == 1 && sensorIndex == 0 && lastObjectFrameTime != sensorData.lastObjectFrameTime)
            {
                // first sensor
                lastObjectFrameTime = sensorData.lastObjectFrameTime;

                prevObjectFrameTicks = lastObjectFrameTicks;
                lastObjectFrameTicks = DateTime.Now.Ticks;

                // take the tracked objects from sensor 0
                trackedObjectsCount = sensorData.trackedObjectsCount;

                if (alTrackedObjects.Length < trackedObjectsCount)
                {
                    Array.Resize<KinectInterop.ObjectData>(ref alTrackedObjects, (int)trackedObjectsCount);

                    for (int i = 0; i < trackedObjectsCount; i++)
                    {
                        alTrackedObjects[i] = new KinectInterop.ObjectData();
                    }
                }

                for (int i = 0; i < trackedObjectsCount; i++)
                {
                    sensorData.alTrackedObjects[i].CopyTo(ref alTrackedObjects[i]);
                }
            }
            else
            {
        ...
        // processes tracked objects
        private void ProcessTrackedObjects()
        {
            List<ulong> addedObjIDs = new List<ulong>();
            List<int> addedIndexes = new List<int>();

            List<ulong> lostObjIDs = new List<ulong>();
            lostObjIDs.AddRange(objectManager.alObjectIds);

            //bLimitedObjects = showAllowedObjectsOnly &&
            //    (maxTrackedObjects > 0 || minUserDistance >= 0.01f || maxUserDistance >= 0.01f || maxLeftRightDistance >= 0.01f);

            for (int i = 0; i < trackedObjectsCount; i++)
            {
                KinectInterop.ObjectData objData = alTrackedObjects[i];
                ulong objId = objData.trackingID;

                if (objData.isTracked && objId != 0)   // && Mathf.Abs(objData.position.z) >= minUserDistance &&
                   //(maxUserDistance < 0.01f || Mathf.Abs(objData.position.z) <= maxUserDistance) &&
                   //(maxLeftRightDistance < 0.01f || Mathf.Abs(objData.position.x) <= maxLeftRightDistance))
                {
                    // add objId to the list of new objects
                    ...
                    lostObjIDs.Remove(objId);
                    objectManager.dictObjectIdToTime[objId] = Time.time;
                }
                else
                {
                    // consider object as not tracked
                    objData.isTracked = false;
                    alTrackedObjects[i] = objData;
                }

                //Debug.Log("  (M)Object ID: " + objId + ", obj: " + i + ", oi: " + objData.objIndex + ", pos: " + objData.position + ", tracked: " + objData.isTracked);
            }

            // remove the lost objects
            if (lostObjIDs.Count > 0)
            {
                foreach (ulong objId in lostObjIDs)
                {
                    // prevent object removal on sporadical tracking failures
                    if ((Time.time - objectManager.dictObjectIdToTime[objId]) >= waitTimeBeforeRemove)
                    {
                        RemoveObject(objId);
                    }
                }
            ...
            if (addedObjIDs.Count > 0)
            {
                // add newly detected objects
                for (int i = 0; i < addedObjIDs.Count; i++)
                {
                    ulong objId = addedObjIDs[i];
                    int objIndex = addedIndexes[i];

                    if (!objectManager.alObjectIds.Contains(objId))
                    {
                        //if (maxTrackedUsers == 0 || objectManager.alObjectIds.Count < maxTrackedUsers)
                        {
                            AddObject(objId, objIndex, alTrackedObjects[objIndex].position);
                        }
                    }
                }

                // update object indices, as needed
                for (int i = 0; i < addedObjIDs.Count; i++)
                {
                    ulong objId = addedObjIDs[i];
                    int objIndex = addedIndexes[i];

                    int oi = objectManager.dictObjectIdToIndex.ContainsKey(objId) ? objectManager.dictObjectIdToIndex[objId] : -1;
                    if (oi >= 0 && oi != objIndex)
                    {
                        // update object index if needed
                        objectManager.dictObjectIdToIndex[objId] = objIndex;

                        //int oidIndex = Array.IndexOf(objectManager.aObjectIndexIds, objId);
                        //if (consoleLogMessages)
                        //    Debug.Log("Updating object " + oidIndex + ", ID: " + objId + ", Obj: " + objIndex + ", Time: " + Time.time);

                        //// print object IDs and indices
                        //objectManager.PrintManagedObjects(trackedObjectsCount, alTrackedObjects);

                        //// re-arrange user indices if needed
                        //objectManager.RearrangeObjectIndices(objectDetectionOrder);
                    }
                }
                ...
                //{
                //    // update object indices, as needed
                //    objectManager.UpdateObjectIndices(objectDetectionOrder);
                //}
        ...
        private void AddObject(ulong objId, int objIndex, Vector3 objPos)
        {
            int oidIndex = objectManager.AddObject(objId, objIndex);

            if (oidIndex >= 0)
            {
                //if (consoleLogMessages)
                //    Debug.Log("Adding object " + oidIndex + ", ID: " + objId + ", Obj: " + objIndex + ", Pos: " + objPos + ", Time: " + objectManager.dictObjectIdToTime[objId]);

                //// print object IDs and indices
                //objectManager.PrintManagedObjects(trackedObjectsCount, alTrackedObjects);

                // fire event
                objectManager.FireOnObjectAdded(objId, oidIndex);
            }
        }
        ...
        {
            int objIndex = objectManager.dictObjectIdToIndex[objId];
            int oidIndex = objectManager.RemoveObject(objId);

            if (oidIndex >= 0)
            {
                //if (consoleLogMessages)
                //    Debug.Log("Removing object " + oidIndex + ", ID: " + objId + ", Obj: " + objIndex + ", Time: " + Time.time);

                //// print object IDs and indices
                //objectManager.PrintManagedObjects(trackedObjectsCount, alTrackedObjects);

                // fire event
                objectManager.FireOnObjectRemoved(objId, oidIndex);
            }
        }
