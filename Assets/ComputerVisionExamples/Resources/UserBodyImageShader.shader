	...
	SubShader {
		Pass {
			...
			float4 frag (v2f i) : COLOR
			{
				...
				//uint bi4 = _BodyIndexMap.Load(uint3(dx >> 4, dy, 0))[(dx >> 2) & 3];
				uint bi = 255;
				...
				float4 clrPlayer = _BodyIndexColors[bi];
				return float4(clrPlayer.rgb, clrPlayer.a && (bi != 255));
			}
