        ...
        void Update()
        {
            ...
            if (kinectManager && kinectManager.IsInitialized() && depthImage /**&& depthImage.sprite == null*/)
            {
                ...
                if (texDepth != null)
                {
                    ...
                    if (sensorData != null && (texDepth2D == null || texDepth2D.width != texDepth.width || texDepth2D.height != texDepth.height))
                    {
                        KinectInterop.Destroy(texDepth2D);
                        texDepth2D = new Texture2D(texDepth.width, texDepth.height, TextureFormat.ARGB32, false);

                        KinectInterop.Destroy(depthImage.sprite);
        ...
        private void SetupJointColliders()
        {
            if (kinectManager && kinectManager.IsInitialized())
            {
                ...
                if (sensorData != null && foregroundCamera != null)
                {
                    ...
                    if (depthImageWidth == 0 || depthImageHeight == 0 || sensorData.depthCamIntr == null)
                        return;
                    ...
                    //Debug.Log($"foregroundImgRect: {foregroundImgRect}");
