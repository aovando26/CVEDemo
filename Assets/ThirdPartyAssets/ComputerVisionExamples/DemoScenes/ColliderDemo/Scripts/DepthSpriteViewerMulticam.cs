            ...
            if (kinectManager && bodyIndex >= 0 && kinectManager.IsSensorBodyDetected(sensorIndex, bodyIndex) && foregroundCamera)
            {
                ...
                for (int i = 0; i < numColliders; i++)
                {
                    ...
                    if (kinectManager.IsSensorJointTracked(sensorIndex, bodyIndex, i))
                    {
                        ...
                        Vector3 posJoint = kinectManager.GetPosDepthOverlay(posJointSensor, sensorIndex, foregroundCamera, foregroundImgRect);
                        ...
                        else
                        {
                            ...
                            if (kinectManager.IsSensorJointTracked(sensorIndex, bodyIndex, p))
                            {
                                ...
                                Vector3 posParent = kinectManager.GetPosDepthOverlay(posParentSensor, sensorIndex, foregroundCamera, foregroundImgRect);
