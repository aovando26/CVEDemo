...
namespace com.rfilkov.components
{
    ...
    public class BackgroundColorCamDepthImage : MonoBehaviour
    {
        ...
        //private Texture2D depthImageBuffer = null;
        //private Texture2D depthHistBuffer = null;
        ...
        void Start()
        {
            ...
            if (sensorData != null)
            {
                ...
                depthHistBuffer = KinectInterop.CreateComputeBuffer(depthHistBuffer, DepthSensorBase.MAX_DEPTH_DISTANCE_MM, sizeof(int));  //  + 1
                //depthHistBuffer = KinectInterop.CreateTexture2D(depthHistBuffer, DepthSensorBase.MAX_DEPTH_DISTANCE_MM >> 2, 1, TextureFormat.RGBAFloat);
        ...
        void OnDestroy()
        {
            ...
            {
                //KinectInterop.Destroy(depthImageBuffer);
            ...
            {
                //KinectInterop.Destroy(depthHistBuffer);
        ...
        void Update()
        {
            if (kinectManager && kinectManager.IsInitialized())
            {
                ...
                if (depthImageTexture != null && backgroundTexture != null)
                {
                    Vector2 imageScale = sensorData.colorImageScale;
                    KinectInterop.TransformTexture(depthImageTexture, backgroundTexture, 0, imageScale.x < 0f, imageScale.y < 0f, backgroundCamera == null);
                }
        ...
        private void UpdateTextureWithNewFrame()
        {
            ...
            if (lastColorCamDepthFrameTime != sensorData.lastColorCamDepthFrameTime)
            {
                ...
                if (depthImageTexture == null || depthImageTexture.width != sensorData.colorImageWidth || depthImageTexture.height != sensorData.colorImageHeight)
                {
                    //Debug.Log($"Creating depthImageTexture - colorImageWidth: {sensorData.colorImageWidth}, colorImageHeight: {sensorData.colorImageHeight}");
                    depthImageTexture = KinectInterop.CreateRenderTexture(depthImageTexture, sensorData.colorImageWidth, sensorData.colorImageHeight);
                }
                ...
                if(depthImageBuffer == null || depthImageBuffer.count != depthBufferLength)  // depthImageBuffer.width != (sensorData.colorImageWidth >> 3) || depthImageBuffer.height != sensorData.colorImageHeight)
                {
                    depthImageBuffer = KinectInterop.CreateComputeBuffer(depthImageBuffer, depthBufferLength, sizeof(uint));
                    //depthImageBuffer = KinectInterop.CreateTexture2D(depthImageBuffer, sensorData.colorImageWidth >> 3, sensorData.colorImageHeight, TextureFormat.RGBAFloat);
                }

                KinectInterop.SetComputeBufferData(depthImageBuffer, sensorData.colorCamDepthImage, depthBufferLength, sizeof(uint));
                //KinectInterop.SetTextureData(depthImageBuffer, sensorData.colorCamDepthImage, sensorData.colorCamDepthImage.Length >> 1, sizeof(uint));

                if (depthHistBuffer != null)
                {
                    KinectInterop.SetComputeBufferData(depthHistBuffer, equalHistBufferData, equalHistBufferData.Length, sizeof(int));
                    //KinectInterop.SetTextureData(depthHistBuffer, equalHistBufferData, equalHistBufferData.Length, sizeof(uint));
                }
                ...
                //Debug.Log($"BackgroundColorCamDepthImage:UpdateTextureWithNewFrame, frame: {kinect.UniCamInterface._frameIndex}");
            }
