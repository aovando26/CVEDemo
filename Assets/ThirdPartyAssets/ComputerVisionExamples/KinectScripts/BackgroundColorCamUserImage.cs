...
namespace com.rfilkov.components
{
    ...
    public class BackgroundColorCamUserImage : MonoBehaviour
    {
        ...
        //private Texture2D bodyIndexBuffer = null;
        ...
        void Start()
        {
            ...
            if(sensorData != null)
            {
                ...
                bodyImageMaterial = new Material(Shader.Find("Kinect/UserBodyImageShader"));  // UserHistImageShader
        ...
        void OnDestroy()
        {
            ...
            {
                //KinectInterop.Destroy(bodyIndexBuffer);
        ...
        void Update()
        {
            if (kinectManager && kinectManager.IsInitialized())
            {
                ...
                if (bodyImageTexture != null && backgroundTexture != null)
                {
                    Vector2 imageScale = sensorData.colorImageScale;
                    KinectInterop.TransformTexture(bodyImageTexture, backgroundTexture, 0, imageScale.x < 0f, imageScale.y < 0f, backgroundCamera == null);
                }
        ...
        private void UpdateTextureWithNewFrame()
        {
            if (sensorData == null || sensorData.sensorInterface == null || sensorData.colorCamBodyIndexImage == null)  // || sensorData.colorCamDepthImage == null)
                return;
            if (sensorData.colorImageWidth == 0 || sensorData.colorImageHeight == 0 || sensorData.lastColorCamBodyIndexFrameTime == 0)  // || sensorData.lastColorCamDepthFrameTime == 0)
                return;
            ...
            if (/**lastColorCamDepthFrameTime != sensorData.lastColorCamDepthFrameTime ||*/ lastColorCamBodyIndexFrameTime != sensorData.lastColorCamBodyIndexFrameTime)
            {
                ...
                if (bodyIndexBuffer == null || bodyIndexBuffer.count != bodyIndexBufferLength)  // bodyIndexBuffer.width != (sensorData.colorImageWidth >> 4) || bodyIndexBuffer.height != sensorData.colorImageHeight)
                {
                    bodyIndexBuffer = KinectInterop.CreateComputeBuffer(bodyIndexBuffer, bodyIndexBufferLength, sizeof(uint));
                    //bodyIndexBuffer = KinectInterop.CreateTexture2D(bodyIndexBuffer, sensorData.colorImageWidth >> 4, sensorData.colorImageHeight, TextureFormat.RGBAFloat);
                }

                KinectInterop.SetComputeBufferData(bodyIndexBuffer, sensorData.colorCamBodyIndexImage, bodyIndexBufferLength, sizeof(uint));
                //bodyIndexBuffer.LoadRawTextureData(sensorData.colorCamBodyIndexImage);
                //bodyIndexBuffer.Apply();
                ...
                //Debug.Log($"BackgroundColorCamUserImage:UpdateTextureWithNewFrame, frame: {kinect.UniCamInterface._frameIndex}");
            }
        }
