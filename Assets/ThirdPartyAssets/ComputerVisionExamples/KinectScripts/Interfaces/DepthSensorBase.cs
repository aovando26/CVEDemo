...
namespace com.rfilkov.kinect
{
    ...
    public abstract class DepthSensorBase : MonoBehaviour, DepthSensorInterface
    {
        ...
        [HideInInspector]
        public bool isSyncDepthAndColor = false;
        [HideInInspector]
        public bool isSyncBodyAndDepth = false;
        ...
        [HideInInspector]
        public Vector3 initialPosePosition = Vector3.zero;
        [HideInInspector]
        public Quaternion initialPoseRotation = Quaternion.identity;
        ...
        //[HideInInspector]
        protected Matrix4x4 depth2colorCamMat = Matrix4x4.identity;
        ...
        // body tracker
        [HideInInspector]
        public bool bIgnoreZCoordinates = false;
        [HideInInspector]
        public bool bIgnoreInferredJoints = false;
        ...
        // raw object data
        protected byte[] rawObjectIndexImage = null;
        protected uint trackedObjectsCount = 0;
        protected List<KinectInterop.ObjectData> alTrackedObjects = null;
        protected ulong rawObjectTimestamp = 0;
        protected ulong rawObjectIndexTimestamp = 0;
        protected ulong currentObjectTimestamp = 0;
        protected ulong currentObjectIndexTimestamp = 0;
        public object objectTrackerLock = new object();
        ...
        // color cam object index frame
        protected byte[] colorCamObjIndexFrame = null;
        protected ulong lastColorCamObjIndexFrameTime = 0;
        protected object colorCamObjIndexFrameLock = new object();
        ...
        // color cam object index shader
        protected bool colorObjIndexShaderInited = false;
        protected ComputeBuffer objectIndexBuffer = null;
        ...
        /// <summary>
        /// Sets depth-and-color frame synchronization flag.
        /// </summary>
        /// <param name="bSyncDepthAndColor">Depth-and-color sync flag</param>
        public void SetDepthAndColorSync(bool bSyncDepthAndColor, bool bIgnoreStreams = false)
        {
            isSyncDepthAndColor = bSyncDepthAndColor && (bIgnoreStreams || (  /**((frameSourceFlags & KinectInterop.FrameSource.TypeColor) != 0) &&*/
                ((frameSourceFlags & KinectInterop.FrameSource.TypeDepth) != 0)));
        }

        /// <summary>
        /// Sets body-and-depth frame synchronization flag.
        /// </summary>
        /// <param name="bSyncBodyAndDepth">Body-and-depth sync flag</param>
        public void SetBodyAndDepthSync(bool bSyncBodyAndDepth, bool bIgnoreStreams = false)
        {
            isSyncBodyAndDepth = bSyncBodyAndDepth && (bIgnoreStreams || (((frameSourceFlags & (KinectInterop.FrameSource.TypeBody | KinectInterop.FrameSource.TypeBodyIndex | KinectInterop.FrameSource.TypeObject | KinectInterop.FrameSource.TypeObjectIndex)) != 0) /**&&
                ((frameSourceFlags & (KinectInterop.FrameSource.TypeColor | KinectInterop.FrameSource.TypeDepth)) != 0)*/));
        }
        ...
        public virtual KinectInterop.SensorData OpenSensor(KinectManager kinectManager, KinectInterop.FrameSource dwFlags, bool bSyncDepthAndColor, bool bSyncBodyAndDepth)
        {
            ...
            isSyncDepthAndColor = bSyncDepthAndColor && /**((dwFlags & KinectInterop.FrameSource.TypeColor) != 0) &&*/ ((dwFlags & KinectInterop.FrameSource.TypeDepth) != 0);  // color is always available
            isSyncBodyAndDepth = bSyncBodyAndDepth && ((dwFlags & (KinectInterop.FrameSource.TypeBody | KinectInterop.FrameSource.TypeBodyIndex | KinectInterop.FrameSource.TypeObject | KinectInterop.FrameSource.TypeObjectIndex)) != 0) /**&&
                ((dwFlags & (KinectInterop.FrameSource.TypeColor | KinectInterop.FrameSource.TypeDepth)) != 0)*/;
        ...
        public virtual void CloseSensor(KinectInterop.SensorData sensorData)
        {
            ...
            rawObjectTimestamp = rawObjectIndexTimestamp = 0;
            rawPoseTimestamp = 0;

            // stop body tracking, if needed
            StopBodyTracking(sensorData);

            // stop object tracking, if needed
            StopObjectTracking(sensorData);
            ...
            DisposeColorObjectIndexShader(sensorData);
        ...
        public virtual void InitSensorData(KinectInterop.SensorData sensorData, KinectManager kinectManager)
        {
            ...
            //    kinectManager.getObjectFrames == KinectManager.ObjectTrackingType.SegmentationTexture)
            //{
        ...
        public virtual void PollSensorFrameTimes(KinectInterop.SensorData sensorData)
        {
            ...
            if (currentObjectTimestamp != rawObjectTimestamp)
            {
                // new object frame
                currentObjectTimestamp = rawObjectTimestamp;
            }

            if (currentObjectIndexTimestamp != rawObjectIndexTimestamp)
            {
                // new object-index frame
                currentObjectIndexTimestamp = rawObjectIndexTimestamp;
            }
        }
        ...
        public virtual bool UpdateSensorData(KinectInterop.SensorData sensorData, KinectManager kinectManager, bool isPlayMode)
        {
            ...
            // body frame
            if(alTrackedBodies != null && sensorData.lastBodyFrameTime != currentBodyTimestamp)
            {
                lock (bodyTrackerLock)
                {
                    // number of bodies
            ...
            // body index frame
            if (rawBodyIndexImage != null && sensorData.lastBodyIndexFrameTime != currentBodyIndexTimestamp)
            {
                lock (bodyTrackerLock)
                {
                    // body index image
            ...
            // object frame
            if (alTrackedObjects != null && sensorData.lastObjectFrameTime != currentObjectTimestamp)
            {
                lock (objectTrackerLock)
                {
                    // number of objects
                    sensorData.trackedObjectsCount = trackedObjectsCount;
                    ...
                    for (int i = 0; i < trackedObjectsCount; i++)
                    {
                        alTrackedObjects[i].CopyTo(ref sensorData.alTrackedObjects[i]);

                        KinectInterop.ObjectData objData = sensorData.alTrackedObjects[i];
                        //Debug.Log("  Object ID: " + objData.trackingID + ", obj: " + i + ", oi: " + objData.objIndex + ", pos: " + objData.kinectPos);
                    }

                    sensorData.lastObjectFrameTime = currentObjectTimestamp;
                    //Debug.Log("D" + deviceIndex + " UpdateObjectTimestamp: " + currentObjectTimestamp + ", ObjCount: " + trackedObjectsCount + ", Now: " + DateTime.Now.Ticks /*.ToString("HH:mm:ss.fff")*/);
                }
            }

            // object-index frame
            if (rawObjectIndexImage != null && sensorData.lastObjectIndexFrameTime != currentObjectIndexTimestamp)
            {
                lock (objectTrackerLock)
                {
                    // body index image
                    if (sensorData.objectIndexImage != null)
                    {
                        KinectInterop.CopyBytes(rawObjectIndexImage, sizeof(byte), sensorData.objectIndexImage, sizeof(byte));
                    }

                    sensorData.lastObjectIndexFrameTime = currentObjectIndexTimestamp;
                    //Debug.Log("D" + deviceIndex + " UpdateObjectIndexTimestamp: " + currentObjectIndexTimestamp + ", Now: " + DateTime.Now.ToString("HH:mm:ss.fff"));
                }
            }
        ...
        protected virtual bool UpdateColorDepthShader(KinectInterop.SensorData sensorData)
        {
            if (colorCamDepthDataFrame != null)
            {
                    ...
                    sensorData.lastColorDepthBufferTime != lastColorCamDepthFrameTime)
                {
                    ...
                    //Debug.Log($"UpdateColorDepthShader, frame: {kinect.UniCamInterface._frameIndex}");
                }
        ...
        protected virtual bool UpdateColorBodyIndexShader(KinectInterop.SensorData sensorData)
        {
            if (colorCamBodyIndexFrame != null)
            {
                    ...
                    sensorData.lastColorBodyIndexBufferTime != lastColorCamBodyIndexFrameTime)
                {
                    ...
                    //Debug.Log($"UpdateColorBodyIndexShader, frame: {kinect.UniCamInterface._frameIndex}");
                }
        ...
        protected virtual bool CreateColorObjectIndexShader(KinectInterop.SensorData sensorData)
        {
            if (colorCamObjIndexFrame == null)
            {
                colorCamObjIndexFrame = new byte[sensorData.colorImageWidth * sensorData.colorImageHeight];
            }

            if (sensorData.colorObjectIndexTexture == null)
            {
                sensorData.colorObjectIndexTexture = KinectInterop.CreateRenderTexture(sensorData.colorObjectIndexTexture, sensorData.colorImageWidth, sensorData.colorImageHeight, RenderTextureFormat.ARGB32);
                //sensorData.colorObjectIndexTexture.enableRandomWrite = true;
                sensorData.colorObjectIndexTexture.Create();
            }

            colorObjIndexShaderInited = true;

            return true;
        }


        // disposes the color-camera object index shader and its respective buffers
        protected virtual void DisposeColorObjectIndexShader(KinectInterop.SensorData sensorData)
        {
            if (colorCamObjIndexFrame != null)
            {
                colorCamObjIndexFrame = null;
            }

            if (sensorData.colorObjectIndexTexture != null)
            {
                sensorData.colorObjectIndexTexture.Release();
                sensorData.colorObjectIndexTexture = null;
            }
            ...
            //if (colorObjectIndexShader != null)
            //{
            //    colorObjectIndexShader = null;
            //}

            colorObjIndexShaderInited = false;
        }


        // updates the color-camera object index shader with the actual data
        protected virtual bool UpdateColorObjectIndexShader(KinectInterop.SensorData sensorData)
        {
            if (colorCamObjIndexFrame != null)
            {
                if (sensorData.lastColorObjectIndexBufferTime != lastColorCamObjIndexFrameTime)
                {
                    lock (colorCamObjIndexFrameLock)
                    {
                        sensorData.lastColorObjectIndexBufferTime = lastColorCamObjIndexFrameTime;

                        if (sensorData.colorImageTexture != null)
                        {
                            if (sensorData.colorObjectIndexTexture == null || sensorData.colorObjectIndexTexture.width != sensorData.colorImageWidth || sensorData.colorObjectIndexTexture.height != sensorData.colorImageHeight)
                            {
                                sensorData.colorObjectIndexTexture = KinectInterop.CreateRenderTexture(sensorData.colorObjectIndexTexture, sensorData.colorImageWidth, sensorData.colorImageHeight, RenderTextureFormat.ARGB32);
                                sensorData.colorObjectIndexTexture.Create();
                            }

                            Graphics.Blit(sensorData.colorImageTexture, sensorData.colorObjectIndexTexture);
                        }

                        if (sensorData.colorObjectIndexBuffer != null)
                        {
                            int bufferLength = sensorData.colorImageWidth * sensorData.colorImageHeight >> 2;
                            if (sensorData.colorObjectIndexBuffer.count != bufferLength)
                            {
                                sensorData.colorObjectIndexBuffer = KinectInterop.CreateComputeBuffer(sensorData.colorObjectIndexBuffer, bufferLength, sizeof(uint));
                            }

                            KinectInterop.SetComputeBufferData(sensorData.colorObjectIndexBuffer, colorCamObjIndexFrame, bufferLength, sizeof(uint));
                        }

                        if (sensorData.colorCamObjectIndexImage != null)
                        {
                            if (sensorData.colorCamObjectIndexImage.Length != colorCamObjIndexFrame.Length)
                            {
                                sensorData.colorCamObjectIndexImage = new byte[colorCamObjIndexFrame.Length];
                            }

                            KinectInterop.CopyBytes(colorCamObjIndexFrame, sizeof(byte), sensorData.colorCamObjectIndexImage, sizeof(byte));
                            sensorData.lastColorCamObjectIndexFrameTime = lastColorCamObjIndexFrameTime;
                            //Debug.Log($"lastColorCamObjectIndexFrameTime: {sensorData.lastColorCamObjectIndexFrameTime}");
                        }
                    }
                }
        ...
        protected virtual void DisposeDepthTexShader(KinectInterop.SensorData sensorData)
        {
            ...
            if (sensorData.depthImageBuffer != null)
            {
                //KinectInterop.Destroy(sensorData.depthImageBuffer);
        ...
        public virtual bool UpdateTransformedFrameTextures(KinectInterop.SensorData sensorData, KinectManager kinectManager)
        {
            ...
            // color cam object-index frame
            if (sensorData.colorObjectIndexBuffer != null || sensorData.colorCamObjectIndexImage != null)
            {
                if (colorObjIndexShaderInited || CreateColorObjectIndexShader(sensorData))
                {
                    UpdateColorObjectIndexShader(sensorData);
                }
            }
            else
            {
                if (colorObjIndexShaderInited)
                {
                    DisposeColorObjectIndexShader(sensorData);
                }
            }
        ...
        public virtual bool UpdateSensorTextures(KinectInterop.SensorData sensorData, KinectManager kinectManager, ulong prevDepthFrameTime, ulong prevIrFrameTime)
        {
            ...
            {
                // depth texture
                if (kinectManager.getDepthFrames == KinectManager.DepthTextureType.DepthTexture &&  // sensorData.depthImageTexture != null && sensorData.depthImageMaterial != null &&
                ...
                // infrared texture
                if ((kinectManager.getInfraredFrames == KinectManager.InfraredTextureType.InfraredTexture ||  // (sensorData.infraredImageTexture != null && sensorData.infraredImageMaterial != null) ||
                    (sensorData.colorInfraredTexture != null && sensorData.colorInfraredMaterial != null)) &&
                ...
                if (kinectManager.getBodyFrames == KinectManager.BodyTextureType.BodyTexture &&  // sensorData.bodyImageTexture != null && sensorData.bodyImageMaterial != null &&
                ...
                // segmentation texture & object texture
                if (kinectManager.getObjectFrames == KinectManager.ObjectTrackingType.ObjectTexture &&  // sensorData.objectImageTexture != null && sensorData.objectImageMaterial != null &&
                    sensorData.lastObjectImageTime != sensorData.lastObjectIndexFrameTime)
                {
                    UpdateSensorTexObjectIndexImg(sensorData, kinectManager);

                    sensorData.lastObjectImageTime = sensorData.lastObjectIndexFrameTime;
                    //Debug.Log("D" + deviceIndex + " ObjectTextureTimestamp: " + sensorData.lastObjectImageTime);
                }
        ...
        private void UpdateSensorTexDepthImg(KinectInterop.SensorData sensorData, KinectManager kinectManager)
        {
            ...
            if(depthHistBufferData == null)
                depthHistBufferData = new int[MAX_DEPTH_DISTANCE_MM];  //  + 1
            if(equalHistBufferData == null)
                equalHistBufferData = new int[MAX_DEPTH_DISTANCE_MM];  //  + 1
            ...
            if (sensorData.depthImageBuffer == null || sensorData.depthImageBuffer.count != depthBufferLength)  // sensorData.depthImageBuffer.width != (sensorData.depthImageWidth >> 3) || sensorData.depthImageBuffer.height != sensorData.depthImageHeight)
            {
                sensorData.depthImageBuffer = KinectInterop.CreateComputeBuffer(sensorData.depthImageBuffer, depthBufferLength, sizeof(uint));
                //sensorData.depthImageBuffer = KinectInterop.CreateTexture2D(sensorData.depthImageBuffer, sensorData.depthImageWidth >> 3, sensorData.depthImageHeight, TextureFormat.RGBAFloat);
            }

            if (sensorData.depthImageBuffer != null && sensorData.depthImage != null)
            {
                if (sensorData.depthImageBuffer.count == (sensorData.depthImage.Length >> 1))
                    KinectInterop.SetComputeBufferData(sensorData.depthImageBuffer, sensorData.depthImage, depthBufferLength, sizeof(uint));
                //KinectInterop.SetTextureData(sensorData.depthImageBuffer, sensorData.depthImage, sensorData.depthImage.Length >> 1, sizeof(uint));
            }

            if (sensorData.depthHistBuffer == null || sensorData.depthHistBuffer.count != MAX_DEPTH_DISTANCE_MM)  // sensorData.depthHistBuffer.width != (MAX_DEPTH_DISTANCE_MM >> 2) || sensorData.depthImageBuffer.height != 1)
            {
                sensorData.depthHistBuffer = KinectInterop.CreateComputeBuffer(sensorData.depthHistBuffer, MAX_DEPTH_DISTANCE_MM, sizeof(uint));
                //sensorData.depthHistBuffer = KinectInterop.CreateTexture2D(sensorData.depthHistBuffer, MAX_DEPTH_DISTANCE_MM >> 2, 1, TextureFormat.RGBAFloat);
            }

            if (sensorData.depthHistBuffer != null && equalHistBufferData != null)
            {
                KinectInterop.SetComputeBufferData(sensorData.depthHistBuffer, equalHistBufferData, equalHistBufferData.Length, sizeof(int));
                //KinectInterop.SetTextureData(sensorData.depthHistBuffer, equalHistBufferData, equalHistBufferData.Length, sizeof(uint));
            }
            ...
            {
                sensorData.depthImageTexture = KinectInterop.CreateRenderTexture(sensorData.depthImageTexture, sensorData.depthImageWidth, sensorData.depthImageHeight);
            }

            if (sensorData.depthImageMaterial == null)
            {
                Shader depthImageShader = Shader.Find("Kinect/DepthHistImageShader");
                sensorData.depthImageMaterial = depthImageShader != null ? new Material(depthImageShader) : null;
            }
            ...
            //Debug.Log($"UpdateSensorTexDepthImg, frame: {kinect.UniCamInterface._frameIndex}");
        }
        ...
        private void UpdateSensorTexInfraredImg(KinectInterop.SensorData sensorData, KinectManager kinectManager)
        {
            ...
            {
                int infraredBufferLength = (sensorData.depthImageWidth * sensorData.depthImageHeight) >> 1;
                if (sensorData.infraredImageBuffer == null || sensorData.infraredImageBuffer.count != infraredBufferLength)
                {
                    sensorData.infraredImageBuffer = KinectInterop.CreateComputeBuffer(sensorData.infraredImageBuffer, infraredBufferLength, sizeof(uint));
                }

                if (sensorData.infraredImageBuffer != null && sensorData.infraredImage != null)
                {
                    if (sensorData.infraredImageBuffer.count == (sensorData.infraredImage.Length >> 1))
                ...
                {
                    sensorData.infraredImageTexture = KinectInterop.CreateRenderTexture(sensorData.infraredImageTexture, sensorData.depthImageWidth, sensorData.depthImageHeight);
                }

                if (sensorData.infraredImageMaterial == null)
                {
                    Shader irImageShader = Shader.Find("Kinect/InfraredImageShader");
                    sensorData.infraredImageMaterial = irImageShader != null ? new Material(irImageShader) : null;
                }
            ...
            {
                if (sensorData.colorInfraredTexture == null || sensorData.colorInfraredTexture.width != sensorData.colorImageWidth || sensorData.colorInfraredTexture.height != sensorData.colorImageHeight)
                {
                    sensorData.colorInfraredTexture = KinectInterop.CreateRenderTexture(sensorData.colorInfraredTexture, sensorData.colorImageWidth, sensorData.colorImageHeight);
                }
        ...
        private void UpdateSensorTexBodyIndexImg(KinectInterop.SensorData sensorData, KinectManager kinectManager)
        {
            ...
            if (sensorData.bodyIndexBuffer == null || sensorData.bodyIndexBuffer.count != bodyIndexBufferLength)  // sensorData.bodyIndexBuffer.width != (sensorData.depthImageWidth >> 4) || sensorData.bodyIndexBuffer.height != sensorData.depthImageHeight)
            {
                ...
                //sensorData.bodyIndexBuffer = KinectInterop.CreateTexture2D(sensorData.bodyIndexBuffer, sensorData.depthImageWidth >> 4, sensorData.depthImageHeight, TextureFormat.RGBAFloat);
            }

            if (sensorData.bodyIndexBuffer != null && sensorData.bodyIndexImage != null)  // && (sensorData.bodyIndexImage.Length >> 2) == sensorData.bodyIndexBuffer.count)
            {
                if (sensorData.bodyIndexBuffer.count == (sensorData.bodyIndexImage.Length >> 2))
                ...
                //sensorData.bodyIndexBuffer.LoadRawTextureData(sensorData.bodyIndexImage);
                //sensorData.bodyIndexBuffer.Apply();
            }
            ...
            {
                sensorData.bodyImageTexture = KinectInterop.CreateRenderTexture(sensorData.bodyImageTexture, sensorData.depthImageWidth, sensorData.depthImageHeight);
            }

            if(sensorData.bodyImageMaterial == null)
            {
                Shader bodyImageShader = Shader.Find("Kinect/UserBodyImageShader");
            ...
            if(sensorData.bodyImageMaterial != null)
            {
                ...
                //{
                //    int depthBufferLength = (sensorData.depthImageWidth * sensorData.depthImageHeight) >> 1;
                //    if (sensorData.depthImageBuffer == null || sensorData.depthImageBuffer.count != depthBufferLength)
                //    {
                //        sensorData.depthImageBuffer = KinectInterop.CreateComputeBuffer(sensorData.depthImageBuffer, depthBufferLength, sizeof(uint));
                //    }

                //    if (sensorData.depthImageBuffer != null && sensorData.depthImage != null && (sensorData.depthImage.Length >> 1) == sensorData.depthImageBuffer.count)
                //    {
                //        KinectInterop.SetComputeBufferData(sensorData.depthImageBuffer, sensorData.depthImage, depthBufferLength, sizeof(uint));
                //    }
                ...
                //Debug.Log($"UpdateSensorTexBodyIndexImg, frame: {kinect.UniCamInterface._frameIndex}");
            }
        }

        // updates sensor object-index image
        private void UpdateSensorTexObjectIndexImg(KinectInterop.SensorData sensorData, KinectManager kinectManager)
        {
            //if (kinectManager.getObjectFrames == KinectManager.ObjectTrackingType.SegmentationTexture)
            //{
            //    // body-image hist data
            //    Array.Clear(depthBodyBufferData, 0, depthBodyBufferData.Length);
            //    Array.Clear(equalBodyBufferData, 0, equalBodyBufferData.Length);
            ...
            //    int frameLen = sensorData.objectIndexImage.Length;
            //    for (int i = 0; i < frameLen; i++)
            //    {
            //        int depth = sensorData.depthImage[i];
            //        int limDepth = (depth >= depthMinDistance && depth <= depthMaxDistance) ? depth : 0;
            ...
            //    if (sensorData.objectHistBuffer != null)
            //    {
            //        KinectInterop.SetComputeBufferData(sensorData.objectHistBuffer, equalBodyBufferData, equalBodyBufferData.Length, sizeof(int));
            //    }
            //}

            int objIndexBufferLength = (sensorData.depthImageWidth * sensorData.depthImageHeight) >> 2;
            if (sensorData.objectIndexBuffer == null || sensorData.objectIndexBuffer.count != objIndexBufferLength)  // sensorData.objectIndexBuffer.width != (sensorData.depthImageWidth >> 4) || sensorData.objectIndexBuffer.height != sensorData.depthImageHeight)
            {
                sensorData.objectIndexBuffer = KinectInterop.CreateComputeBuffer(sensorData.objectIndexBuffer, objIndexBufferLength, sizeof(uint));
                //sensorData.objectIndexBuffer = KinectInterop.CreateTexture2D(sensorData.objectIndexBuffer, sensorData.depthImageWidth >> 4, sensorData.depthImageHeight, TextureFormat.RGBAFloat);
            }

            if (sensorData.objectIndexBuffer != null && sensorData.objectIndexImage != null)  // && (sensorData.objectIndexImage.Length >> 2) == sensorData.objectIndexBuffer.count)
            {
                if(sensorData.objectIndexBuffer.count == (sensorData.objectIndexImage.Length >> 2))
                    KinectInterop.SetComputeBufferData(sensorData.objectIndexBuffer, sensorData.objectIndexImage, objIndexBufferLength, sizeof(uint));
                //sensorData.objectIndexBuffer.LoadRawTextureData(sensorData.objectIndexImage);
                //sensorData.objectIndexBuffer.Apply();
            }

            if (sensorData.objectImageTexture == null || sensorData.objectImageTexture.width != sensorData.depthImageWidth || sensorData.objectImageTexture.height != sensorData.depthImageHeight)
            {
                sensorData.objectImageTexture = KinectInterop.CreateRenderTexture(sensorData.objectImageTexture, sensorData.depthImageWidth, sensorData.depthImageHeight);
            }

            if(sensorData.objectImageMaterial == null)
            {
                Shader objectImageShader = Shader.Find("Kinect/UserBodyImageShader");
                sensorData.objectImageMaterial = new Material(objectImageShader);
            }
            ...
            if(sensorData.objectImageMaterial != null)
            {
                ...
                sensorData.objectImageMaterial.SetInt("_TexResX", sensorData.depthImageWidth);
                sensorData.objectImageMaterial.SetInt("_TexResY", sensorData.depthImageHeight);
                sensorData.objectImageMaterial.SetInt("_MinDepth", (int)(minDist * 1000f));
                sensorData.objectImageMaterial.SetInt("_MaxDepth", (int)(maxDist * 1000f));

                sensorData.objectImageMaterial.SetBuffer("_BodyIndexMap", sensorData.objectIndexBuffer);
                ...
                //    sensorData.objectImageMaterial.SetBuffer("_DepthMap", sensorData.depthImageBuffer);
                //    sensorData.objectImageMaterial.SetBuffer("_HistMap", sensorData.objectHistBuffer);
                //    sensorData.objectImageMaterial.SetInt("_TotalPoints", bodyHistTotalPoints);
                //}

                Color[] objectIndexColors = kinectManager.GetObjectIndexColors();
                sensorData.objectImageMaterial.SetColorArray("_BodyIndexColors", objectIndexColors);

                Graphics.Blit(null, sensorData.objectImageTexture, sensorData.objectImageMaterial);
            }
        }
        ...
        private void UpdateSensorTexDepthHsv(KinectInterop.SensorData sensorData, KinectManager kinectManager)
        {
            ...
            {
                KinectInterop.SetComputeBufferData(sensorData.depthImageBuffer, sensorData.depthImage, depthBufferLength, sizeof(uint));
                //KinectInterop.SetTextureData(sensorData.depthImageBuffer, sensorData.depthImage, sensorData.depthImageWidth >> 1, sizeof(uint));
            }
        ...
        {
            int depthBufferLength = (sensorData.depthImageWidth * sensorData.depthImageHeight) >> 1;
            if (sensorData.infraredImageBuffer == null || sensorData.infraredImageBuffer.count != depthBufferLength)
            {
                sensorData.infraredImageBuffer = KinectInterop.CreateComputeBuffer(sensorData.infraredImageBuffer, depthBufferLength, sizeof(uint));
            }
        ...
        public Quaternion GetSensorRotationNotZFlipped(bool bInverted)
        {
        ...
        public virtual Vector3[] GetDepthCameraSpaceTable(KinectInterop.SensorData sensorData)
        {
            ...
            if (sensorData.depthCamIntr != null && (depth2SpaceTable == null || depth2SpaceTable.Length != depthImageLength))
            {
        ...
        public virtual Vector3[] GetColorCameraSpaceTable(KinectInterop.SensorData sensorData)
        {
            ...
            if (sensorData.colorCamIntr != null && (color2SpaceTable == null || color2SpaceTable.Length != colorImageLength))
            {
        ...
        // initializes the object-data structures and start the object tracking
        public virtual bool InitObjectTracking(KinectInterop.FrameSource dwFlags, KinectInterop.SensorData sensorData)
        {
            if ((dwFlags & KinectInterop.FrameSource.TypeObjectIndex) != 0)
            {
                rawObjectIndexImage = new byte[sensorData.depthImageWidth * sensorData.depthImageHeight];
                sensorData.objectIndexImage = new byte[sensorData.depthImageWidth * sensorData.depthImageHeight];
            }

            if ((dwFlags & KinectInterop.FrameSource.TypeObject) != 0)  // check for object stream
            {
                alTrackedObjects = new List<KinectInterop.ObjectData>();
                sensorData.alTrackedObjects = new KinectInterop.ObjectData[0]; // new List<KinectInterop.BodyData>();

                trackedObjectsCount = 0;
                sensorData.trackedObjectsCount = 0;
            }

            return true;
        }

        // stops the object tracker and releases its data
        public virtual void StopObjectTracking(KinectInterop.SensorData sensorData)
        {
        }

        // estimates additional joints for the given body
        public virtual void CalcBodySpecialJoints(ref KinectInterop.BodyData bodyData)
        {
            // hand left
        ...
        public virtual void CalcBodyJointOrients(ref KinectInterop.BodyData bodyData)
        {
                    ...
                    if (nextJoint != joint && nextJoint >= 0 && nextJoint < jointCount)
                    {
                            ...
                            (joint == (int)KinectInterop.JointType.Neck))
                        {
                            if(/**bNextJointValid &&*/ jointDir != Vector3.zero)
                            {
                                ...
                                Vector3 jointDir2 = shouldersDirection.normalized;
                            ...
                            (joint == (int)KinectInterop.JointType.KneeLeft) || (joint == (int)KinectInterop.JointType.KneeRight))
                        {
                            ...
                            {
                                Vector3 baseDir2 = Vector3.right;
                                Vector3 jointDir2 = hipsDirection.normalized;
                                jointDir2.z = -jointDir2.z;
        ...
        public virtual Texture GetDepthCameraColorFrameTexture(KinectInterop.SensorData sensorData, ref Texture2D copyToTex2D, ref ulong frameTime)
        {
            ...
            lock (depthCamColorFrameLock)
            {
                ...
                {
                    KinectInterop.RenderTex2Tex2D((RenderTexture)sensorData.depthCamColorImageTexture, ref copyToTex2D);
        ...
        // enables or disables color camera object-index frame processing
        public virtual void EnableColorCameraObjectIndexFrame(KinectInterop.SensorData sensorData, bool isEnable)
        {
            if (isEnable && colorCamObjIndexFrame == null)
            {
                colorCamObjIndexFrame = new byte[sensorData.colorImageWidth * sensorData.colorImageHeight];
                sensorData.colorCamObjectIndexImage = new byte[sensorData.colorImageWidth * sensorData.colorImageHeight];
            }
            else if (!isEnable && colorCamObjIndexFrame != null)
            {
                colorCamObjIndexFrame = null;
                sensorData.colorCamObjectIndexImage = null;
            }
        }

        // returns the latest color camera object-index frame along with the last frame time
        public virtual byte[] GetColorCameraObjectIndexFrame(KinectInterop.SensorData sensorData, ref byte[] copyToFrame, ref ulong frameTime)
        {
            if (sensorData.colorCamObjectIndexImage == null)
                return null;
            ...
            lock (colorCamObjIndexFrameLock)
            {
                if (copyToFrame != null && sensorData.lastColorCamObjectIndexFrameTime != frameTime)
                {
                    KinectInterop.CopyBytes(sensorData.colorCamObjectIndexImage, sizeof(byte), copyToFrame, sizeof(byte));
                }

                frameTime = sensorData.lastColorCamObjectIndexFrameTime;
            }

            return sensorData.colorCamObjectIndexImage;
        }
