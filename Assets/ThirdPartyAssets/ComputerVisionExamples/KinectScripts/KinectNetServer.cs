...
namespace com.rfilkov.kinect
{
    ...
    public class KinectNetServer : MonoBehaviour
    {
        ...
        [Tooltip("Whether to serve body index frames along with the body frames.")]
        private bool serveBodyIndexFrames = true;

        [Tooltip("Whether to serve object index frames along with the object frames.")]
        private bool serveObjectIndexFrames = true;
        ...
        private TcpNetServer objectDataFrameServer = null;
        private TcpNetServer objectIndexFrameServer = null;
        ...
        private TcpNetServer color2objectIndexFrameServer = null;
        ...
        private ulong lastColor2ObjectIndexFrameTime = 0;
        ...
        private ILZ4Compressor objectIndexFrameCompressor = null;
        private ILZ4Compressor color2depthFrameCompressor = null;
        private ILZ4Compressor color2bodyIndexFrameCompressor = null;
        private ILZ4Compressor color2objectIndexFrameCompressor = null;
        ...
        private void InitNetServers(KinectInterop.FrameSource dwFlags)
        {
            try
            {
                ...
                if ((dwFlags & KinectInterop.FrameSource.TypeObject) != 0)
                {
                    objectDataFrameServer = new TcpNetServer(baseListenPort + (int)NetMessageType.ObjectData, "ObjectDataServer", sbConsole);
                    minPort = Mathf.Min(minPort, objectDataFrameServer.serverPort); maxPort = Mathf.Max(maxPort, objectDataFrameServer.serverPort);
                }

                if ((dwFlags & KinectInterop.FrameSource.TypeObjectIndex) != 0 && serveObjectIndexFrames)
                {
                    objectIndexFrameServer = new TcpNetServer(baseListenPort + (int)NetMessageType.ObjectIndex, "ObjectIndexServer", sbConsole);
                    minPort = Mathf.Min(minPort, objectIndexFrameServer.serverPort); maxPort = Mathf.Max(maxPort, objectIndexFrameServer.serverPort);

                    if (compressRawFrames)
                        objectIndexFrameCompressor = LZ4CompressorFactory.CreateNew();
                }
                ...
                if ((dwFlags & KinectInterop.FrameSource.TypeColor) != 0 && (dwFlags & KinectInterop.FrameSource.TypeDepth) != 0 && (dwFlags & KinectInterop.FrameSource.TypeObjectIndex) != 0)
                {
                    color2objectIndexFrameServer = new TcpNetServer(baseListenPort + (int)NetMessageType.Color2ObjectIndex, "TObjectIndex", sbConsole);
                    minPort = Mathf.Min(minPort, color2objectIndexFrameServer.serverPort); maxPort = Mathf.Max(maxPort, color2objectIndexFrameServer.serverPort);

                    if (compressRawFrames)
                        color2objectIndexFrameCompressor = LZ4CompressorFactory.CreateNew();
                }
        ...
        private void CloseNetServers()
        {
            try
            {
                ...
                if (colorFrameServer != null)
                {
                    ...
                    if (colorTex2d != null)
                        KinectInterop.Destroy(colorTex2d);
                }
                ...
                if (infraredFrameServer != null)
                {
                    ...
                    if (infraredTex2d != null)
                        KinectInterop.Destroy(infraredTex2d);
                }
                ...
                if (objectDataFrameServer != null)
                {
                    objectDataFrameServer.Close();
                    objectDataFrameServer = null;
                }

                if (objectIndexFrameServer != null)
                {
                    objectIndexFrameServer.Close();
                    objectIndexFrameServer = null;
                    objectIndexFrameCompressor = null;
                }
                ...
                if (color2objectIndexFrameServer != null)
                {
                    color2objectIndexFrameServer.Close();
                    color2objectIndexFrameServer = null;
                    color2objectIndexFrameCompressor = null;
                }
            ...
            try
            {
                ...
                if (colorFrameServer != null && colorFrameServer.GetConnCount() > 0)
                {
                    if(sensorData.lastColorFrameTime != lastColorFrameTime)
                    {
                        ...
                        if(sensorData.colorImageTexture is RenderTexture)
                        {
                            if (colorTex2d == null || colorTex2d.width != sensorData.colorImageWidth || colorTex2d.height != sensorData.colorImageHeight)
                            {
                                if (colorTex2d != null)
                                    KinectInterop.Destroy(colorTex2d);
                                colorTex2d = new Texture2D(sensorData.colorImageWidth, sensorData.colorImageHeight, sensorData.colorImageFormat, false);
                            }

                            KinectInterop.RenderTex2Tex2D((RenderTexture)sensorData.colorImageTexture, ref colorTex2d);
                        }

                        byte[] btColorData = (colorTex2d != null ? colorTex2d : (Texture2D)sensorData.colorImageTexture).EncodeToJPG();
                ...
                if (infraredFrameServer != null && infraredFrameServer.GetConnCount() > 0)
                {
                    if (sensorData.lastInfraredImageTime != lastInfraredFrameTime)
                    {
                        ...
                        if(infraredTex2d == null || infraredTex2d.width != sensorData.depthImageWidth || infraredTex2d.height != sensorData.depthImageHeight)
                        {
                            if (infraredTex2d != null)
                                KinectInterop.Destroy(infraredTex2d);
                ...
                // object data frame
                if (objectDataFrameServer != null && objectDataFrameServer.GetConnCount() > 0)
                {
                    if (sensorData.lastObjectFrameTime != lastObjectDataFrameTime)
                    {
                        lastObjectDataFrameTime = sensorData.lastObjectFrameTime;

                        float fUnityTime = 0f;
                        string sObjectFrame = KinectInterop.GetObjectFrameAsCsv(ref sensorData.alTrackedObjects,
                            sensorData.trackedObjectsCount, sensorData.lastObjectFrameTime, sensorData.sensorSpaceScale, ref fUnityTime, '\t');
                        byte[] btObjectData = System.Text.Encoding.UTF8.GetBytes(sObjectFrame);

                        NetMessageData message = new NetMessageData(NetMessageType.ObjectData, FrameEncodeType.K4o,
                            sensorData.depthImageWidth, sensorData.depthImageHeight, lastObjectDataFrameTime);
                        message.SetData(btObjectData);

                        objectDataFrameServer.SendMessageToAllConnections(message);
                    }
                }

                // object index frame
                if (objectIndexFrameServer != null && objectIndexFrameServer.GetConnCount() > 0)
                {
                    if (sensorData.lastObjectIndexFrameTime != lastObjectIndexFrameTime)
                    {
                        lastObjectIndexFrameTime = sensorData.lastObjectIndexFrameTime;

                        NetMessageData message = new NetMessageData(NetMessageType.ObjectIndex, FrameEncodeType.Raw,
                            sensorData.depthImageWidth, sensorData.depthImageHeight, lastObjectIndexFrameTime);
                        message.SetData(sensorData.objectIndexImage);

                        if (compressRawFrames)
                        {
                            message.SetCompressor(objectIndexFrameCompressor);
                            message.CompressData();
                        }

                        objectIndexFrameServer.SendMessageToAllConnections(message);
                    }
                }
                ...
                // color2objectIndex frame
                if (color2objectIndexFrameServer != null && color2objectIndexFrameServer.GetConnCount() > 0)
                {
                    if (colorCamObjectIndexFrameData == null)
                    {
                        colorCamObjectIndexFrameData = new byte[sensorData.colorImageWidth * sensorData.colorImageHeight];
                        sensorData.sensorInterface.EnableColorCameraObjectIndexFrame(sensorData, true);
                        //color2objectIndexEnabled = true;
                    }

                    ulong frameTime = 0;
                    byte[] btFrameData = sensorData.sensorInterface.GetColorCameraObjectIndexFrame(sensorData, ref colorCamObjectIndexFrameData, ref frameTime);

                    if (btFrameData != null && frameTime != lastColor2ObjectIndexFrameTime)
                    {
                        lastColor2ObjectIndexFrameTime = frameTime;

                        NetMessageData message = new NetMessageData(NetMessageType.Color2ObjectIndex, FrameEncodeType.Raw,
                            sensorData.colorImageWidth, sensorData.colorImageHeight, frameTime);
                        message.SetData(btFrameData);

                        if (compressRawFrames)
                        {
                            message.SetCompressor(color2objectIndexFrameCompressor);
                            message.CompressData();
                        }

                        color2objectIndexFrameServer.SendMessageToAllConnections(message);
                    }
                }
                else if (colorCamObjectIndexFrameData != null && color2objectIndexFrameServer != null && color2objectIndexFrameServer.GetConnCount() == 0)
                {
                    // disable color2objectIndex frame
                    //color2objectIndexEnabled = false;
                    colorCamObjectIndexFrameData = null;
        ...
        private int GetConnectionsCount()
        {
            ...
            if (objectDataFrameServer != null)
                numConn += objectDataFrameServer.GetConnCount();
            if (objectIndexFrameServer != null)
                numConn += objectIndexFrameServer.GetConnCount();
            if (poseFrameServer != null)
            ...
            if (color2objectIndexFrameServer != null)
                numConn += color2objectIndexFrameServer.GetConnCount();
        ...
        private int GetMessageCount(bool inclCtrlMessages)
        {
            ...
            if (objectDataFrameServer != null && objectDataFrameServer.IsConnected())
                numMessages += objectDataFrameServer.GetMessageCount();
            if (objectIndexFrameServer != null && objectIndexFrameServer.IsConnected())
                numMessages += objectIndexFrameServer.GetMessageCount();
            ...
            if (color2objectIndexFrameServer != null && color2objectIndexFrameServer.IsConnected())
                numMessages += color2objectIndexFrameServer.GetMessageCount();
