        ...
        void Update()
        {
            if(!isCompleted && kinectManager != null && kinectManager.IsInitialized())
            {
                    ...
                    (maxDuration > 0f && (Time.time - startTime) >= maxDuration))
                {
                    ...
                    Debug.Log($"Pose estimation stopped{stopFirst}.\nSensorPos: {sensorData.sensorPosePosition}, SensorRot: {sensorData.sensorPoseRotation.eulerAngles}, ts: {sensorData.lastSensorPoseFrameTime}. StopTime: {Time.time}");
                }
