        ...
        protected virtual void Update()
        {
            if (kinectManager && kinectManager.IsInitialized())
            {
                ...
                if (kinectManager.IsUserDetected(playerIndex))
                {
                    ...
                    {
                        int joint = i;
                        //Debug.Log($"U {userId} {(KinectInterop.JointType)joint} - {kinectManager.GetJointTrackingState(userId, joint)} - pos: {GetJointPosition(userId, joint)}");

                        if (!filteredOutJoints.Contains(joint) && 
                            kinectManager.IsJointTracked(userId, joint))
                        {
                            ...
                            if (lines[i] != null)
                            {
                                ...
                                if (posJoint != Vector3.zero && posParent != Vector3.zero &&
                                    kinectManager.IsJointTracked(userId, jointParent))
                                {
