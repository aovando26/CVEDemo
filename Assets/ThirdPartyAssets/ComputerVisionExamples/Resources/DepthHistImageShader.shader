...
Shader "Kinect/DepthHistImageShader" {
	...
	SubShader {
		Pass {
			...
			//Texture2D<uint4> _DepthMap;
			//Texture2D<uint4> _HistMap;
			...
			float4 frag (v2f i) : COLOR
			{
				...
				//uint depth2 = _DepthMap.Load(uint3(dx >> 3, dy, 0))[(dx >> 1) & 3];
				uint depth = dx & 1 != 0 ? depth2 >> 16 : depth2 & 0xffff;
				depth = (depth >= _MinDepth && depth < _MaxDepth) * depth;

				float histA = 1.0 - ((float)_HistMap[depth] / (float)_TotalPoints);
				//uint hist = _HistMap.Load(uint3(depth >> 2, 0, 0))[depth & 3];
				//float histA = 1.0 - ((float)hist / _TotalPoints);  // _HistMap[depth]
				float4 clr = lerp(_BackColor, _FrontColor, histA);

				//return float4(histA, histA, 0, 1);  // yellow
